...
## Актуальная принятая точка и реальное состояние — 2026-09-22

- Последняя пользовательски принятая база эмуляции BaseConf: **B40**, локальный
  тег `B40-stable`. B40 включает contention/floating bus, живые переходы
  raster/режима/палитры внутри кадра, связь с фазой INT/video и golden-векторы
  всех семи renderer-путей. Обязательные B40 probes прошли, а runtime-приёмка
  выполнена пользователем.
- **Alpha 5** — опубликованный portable prerelease на B40. **Alpha 6** —
  последующий UI/profile-релиз поверх B40; эмуляция BaseConf, видео, звук,
  тайминги и контроллеры в нём не менялись. **Alpha 7** — текущий опубликованный
  portable prerelease поверх этой базы: No Border, изоляция стартового профиля
  `Test.exe` и UI-правки без изменения эмуляции. Alpha 6 остаётся стабильной
  точкой для отката.
- **Лента закрыта полностью.** TAP/TZX остаётся подключённой при переключении
  BaseConf ↔ Pentagon/Spectrum, а скорость воспроизведения определяется активной
  машиной: BaseConf ×8, остальные машины ×1. Повторно открывать этот пункт
  разрешено только при новом воспроизводимом регрессе.
- Жизненный цикл FDD/HDD/SD, status-индикаторы, reset-логика и порты/память/INT
  BaseConf приняты пользовательскими проверками. В Alpha 6 уже входят Load/Eject
  FDD/HDD/SD и F12/Ctrl+F12/Ctrl+Alt+F12 reset-клавиши.
- На дату этой записи **нет известной активной ошибки эмуляции BaseConf**.
  Формулировки старого аудита о точности raster/AVR/flash — это границы модели,
  а не открытые дефекты: возвращаться к ним можно только по конкретному
  воспроизводимому тесту или демке.

### B41 BaseConf CD/DVD ATAPI — локальная тестовая сборка — 2026-09-22

- Реализован только для **ZX-Evo BaseConf** read-only ATAPI CD/DVD как второй
  (IDE Slave) накопитель документированного канала IDE. IDE Master/HDD и ATM
  Turbo в этот этап не менялись. Основание: документация BaseConf описывает
  один IDE-канал с выбором Master/Slave через `HeadAndDrive`; в RTL контроллера
  также предусмотрены оба устройства:
  [BaseConf manual](https://github.com/tslabs/zx-evo/blob/master/pentevo/docs/Baseconf/zxevo_base_configuration.pdf),
  [IDE RTL](https://github.com/tslabs/zx-evo/blob/master/pentevo/fpga/current/common/ide.v).
- В существующей панели `Machine Settings → PentEvo IDE Settings` добавлена
  группа `CD/DVD-ROM (IDE Slave)`: она по умолчанию выключена, позволяет
  выбрать Windows физический или смонтированный виртуальный оптический привод,
  обновить список или отсоединить его через `Eject`. `Eject` отсоединяет только
  от эмулятора — лоток Windows никогда не открывается. Attach/eject/change
  использует тот же холодный цикл, что уже принят для HDD/SD.
- Реализованы IDENTIFY PACKET, INQUIRY, REQUEST SENSE, TEST UNIT READY,
  MODE SENSE (6), START STOP, PREVENT/ALLOW, READ CAPACITY (10), READ (10) и
  READ TOC. Блок — 2048 байт; чтение открывает `\\.\X:` через Win32
  `CreateFile`, поскольку .NET 4 `FileStream` сам по себе не умеет открывать
  такой путь устройства. Записи, CSS/защищённый VideoDVD, аудио-CD и управление
  физическим лотком намеренно не поддерживаются.
- Профиль сохраняет только флаг подключения и букву привода. Если Windows-диск
  исчез до запуска, CD/DVD остаётся отключённым и не мешает запуску эмулятора.
- Проверки: `AtapiCdromProbe-B41` — **15 PASS** на пустом физическом приводе
  и **18 PASS** на смонтированном `timegal.iso`, включая чтение сектора LBA 16;
  полный `Test.exe` прошёл все ULA sanity-проверки, benchmark стандартных
  рендер-профилей — примерно 320–388 мс за 500 кадров. Полная Release-сборка
  прошла без ошибок; остаются только два прежних предупреждения о ruleset.
- Чистый пакет для первой пользовательской runtime-приёмки, не изменяющий Alpha 7:
  `L:\Work_two\ZX\ZXMAK2-v29-ZXEVO-BaseConf-ATAPI-20260922.zip`, SHA-256
  `CCC89C8F4567D8FF61F8D05EEC3F3F9C615A830F1A3848BBC5998248512E1030`.
  В нём 45 runtime-файлов, включая `Test.exe`; нет PDB, CMOS, VMIDE, VIDE,
  логов и образов. До пользовательского запуска с реальным BaseConf-софтом это
  **не релиз и не GitHub-публикация**. Во время этой приёмки обнаружены два
  дефекта, описанные в следующей записи; пакет не считать пригодным для
  дальнейшей проверки CD/DVD.

### B41-R1 BaseConf CD/DVD ATAPI — исправление runtime-дефектов — 2026-09-22

- В реальном Pentagon-загрузчике Time Gal v29 зависал на экране `Loading…`:
  верхний OSD-индикатор IDE появлялся, но не мигал, то есть до чтения блока
  дело не доходило. Документация именно этого загрузчика требует NemoIDE,
  CD/DVD как Slave, `TEST UNIT READY` перед `SET CD SPEED` и полноценный
  `READ TOC`; источник: [Info Guide #09](https://zxpress.ru/en/ezines/info-guide/09/time-gal-the-first-cd-game-for-zx-spectrum-autorun-cd-standard-description-computer-requirements).
- Исправлена совместимость начального протокола: добавлены `MODE SENSE (10)`
  и успешный read-only ответ на `SET CD SPEED (0xBB)`; `READ TOC` теперь
  выдаёт корректные дескрипторы data track 1 и lead-out (format 0), а также
  сведения о единственной активной сессии, начинающейся с LBA 0 (format 1),
  включая MSF-вариант. Неподдержанный ATAPI-пакет пишет в лог лишь код
  команды — для точной диагностики следующей программы без постоянного
  I/O-лога.
- Исправлен `Eject`: при каждой загрузке профиля slave сначала очищается.
  Это важно, потому что менеджер машин переиспользует объект IDE; раньше
  пустой флаг `ideCdConfigured` не стирал старый выбранный привод, и он
  самопроизвольно возвращался после reset/cold restart.
- Проверка `AtapiCdromProbe-B41` расширена на реальный порядок Time Gal
  `TEST UNIT READY → SET CD SPEED → READ TOC format 0/1 → READ(10)` и на отключение
  CD/DVD при повторной загрузке конфигурации в тот же объект IDE. На
  смонтированном `timegal.iso` — **25 PASS**. Полная Release-сборка после
  правки прошла без ошибок; остаются только два старых warnings о ruleset.
- Эта запись пока **не означает пользовательскую runtime-приёмку**:
  требуется новый чистый тестовый пакет и проверка именно Pentagon-загрузчика
  Time Gal. Alpha 7, GitHub-релизы и принятая эмуляция остальных машин не
  изменяются.
- Для повторной проверки собран отдельный локальный пакет v30:
  `L:\Work_two\ZX\ZXMAK2-v30-ZXEVO-BaseConf-ATAPI-R1-20260922.zip`
  (45 runtime-файлов, SHA-256
  `0C553FA44419919FC28C08C56C1F05F099031D2ECDBFBDC289D81B90DF18FEB4`).
  В нём нет PDB, логов, образов, CMOS, VMIDE или VIDE; v29 сохранён как
  воспроизводимый пакет с обнаруженными дефектами и не перезаписывался.

### Отложенные отдельные функции — не ошибки B40

1. **ULAplus** BaseConf: первая совмещённая B39-попытка была отклонена;
   официальная 4:4:4-палитра принята отдельно в B39A. ULAplus требует своего
   изолированного этапа и не должен смешиваться с уже принятым видеоядром.
2. **ZX-BUS**: два документированных слота и периферийные карты — отдельная
   будущая задача после решения по объёму работы.
3. **CD/ATAPI для BaseConf**: B41-R1 ожидает повторной пользовательской
   runtime-приёмки с Pentagon-загрузчиком Time Gal. ATM Turbo в эту задачу не
   включается: его эмуляция пока не менялась и будет отдельным будущим циклом.
4. Автозагрузка NedoOS с HDD — удобство после отдельной стабилизации и проверки,
   не дефект существующего IDE-контроллера.
5. **Утилита создания SD/IDE-образов**: IMA/IMG/raw/VHD/HDD с явным выбором
   размера, назначения и layout. Эталонные параметры и проверка записаны ниже
   в разделе «Эталон носителей для будущей утилиты». Диагностика Rage закрыта:
   он проходит на корректно созданных 500-МБ и настоящих 4-ГиБ SD-образах, а
   также на 500-МБ IDE/HDD. Не открывать этот пункт снова без нового точного
   воспроизведения.

### Эталон носителей для будущей утилиты

- **SD, 500 МБ:** raw FAT32 без MBR, filesystem начинается с LBA 0; размер
  `524288000` байт (`1024000` секторов по 512 байт), 8 секторов/кластер,
  reserved=32, две FAT по 1000 секторов.
- **SD, настоящий 4 ГиБ:** raw FAT32 без MBR, `4294967296` байт
  (`8388608` секторов), 8 секторов/кластер, reserved=32, две FAT по 8177
  секторов, root cluster=2, label `ZXEVO4G`. Не использовать прежний sparse
  `RAGE-4GB-rawfat32-SDHC.ima` как шаблон: это диагностическая имитация, а не
  полноценный 4-ГиБ FAT32-носитель.
- **IDE/HDD, 500 МБ:** raw IDE image `524288000` байт; MBR, один LBA FAT32
  раздел type `0C`, start LBA 63, total 1023937 sectors; в разделе 512
  байт/сектор, 8 секторов/кластер, reserved=32, две FAT по 998 секторов.
- Порядок операции: создать именно выбранный контейнер и layout, временно
  смонтировать через ImDisk только для записи файлов, отсоединить том, затем
  проверить boot signature `55AA` и повторно прочитать файлы с хэшами. Суффикс
  файла сам по себе не задаёт формат: raw-файл с `.vhd` не является настоящим
  VHD; для VHD нужен корректный footer/тип.
- Это спецификация будущей утилиты, а не специальное исправление для Rage.
  Подробный исторический паспорт всех контрольных файлов сохранён ниже в этом
  же журнале.

### Рабочее правило для следующего изменения

Перед новым этапом перечитать этот раздел, `FORK_CHANGELOG.md` и относящийся к
задаче фрагмент `BASECONF_AUDIT_B32.md`. Не возобновлять закрытую ленту, B40
timing/video или принятый media lifecycle без нового точного воспроизведения
проблемы. Не смешивать задачи ATM Turbo с BaseConf.
...
### Историческое создание контрольных носителей — 2026-09-21
...
- На момент записи 64-МБ Nemo IDE HDD не был эталоном: Rage запускался с него,
  но аварийно завершался после первой части. Позднее вопрос закрыт созданием и
  пользовательской проверкой корректного 500-МБ IDE/HDD; старый образ может
  отличаться только собственной разметкой или содержимым и не является общим
  дефектом IDE или Rage.
...
## GitHub prerelease — ZX-Evo BaseConf Alpha 5 — B40 — 2026-09-22

- По явному запросу пользователя стабильная B40 публикуется как portable
  prerelease `v0.5-alpha`, с одним корневым каталогом
  `ZXMAK2-ZXEvo-BaseConf-Alpha-5` в ZIP.
- В архив входят только runtime-файлы, чистый BaseConf-профиль, ROM/PAK,
  лицензия и актуальная документация. Исключены `.cmos`, `.vmz` кроме чистого
  профиля, `.vmide`, `.nvram`, носители, логи, PDB и development probes.
- Названия существующих GitHub prerelease приведены к общему формату:
  `ZX-Evo BaseConf Alpha 3 — B36`, `Alpha 4 — B38`, `Alpha 5 — B40`.

## Post-Alpha 5 host-interface polish — 2026-09-22

- Работа начата только в отдельной ветке от контрольной точки Alpha 5/B40;
  базовые BaseConf-порты, видео, звуковой тракт, тайминги и контроллеры не
  менялись. До начала правок создан тег `post-alpha5-polish-start`.
- SD теперь хранит путь подключённой карты в конфигурации машины и
  автоматически восстанавливает её при следующем запуске. В Machine Settings
  добавлена страница SD с подключением, выбором файла и Eject; смена либо
  извлечение проходит через тот же контролируемый cold restart, что и HDD.
- Добавлен единый host-слой операций для SD/HDD/FDD. Все три будущие кнопки
  панели и настройки используют одни команды: SD/HDD после смены выполняют
  сохранение конфигурации и cold restart, FDD сохраняет конфигурацию без
  навязанного restart — как существующие FDD settings. Новый FDD по умолчанию
  writable.
- В главном окне добавлены выпадающие индикаторы FDD/HDD/SD: зелёная точка
  означает подключённый носитель, красная — отсутствие. Диаметр точки — одна
  константа `MediaStatusDotDiameter = 10`, чтобы размер можно было быстро
  подстроить после визуальной проверки.
- Добавлены host-only клавиши: F12 — immediate Warm Reset, Ctrl+F12 —
  подтверждённый reset CMOS с timestamped backup, Ctrl+Alt+F12 —
  подтверждённый reset сохранённого machine state с backup и restart.
  Alt+Ctrl+Insert сохранён; Alt+Ctrl+End добавлен как равнозначная альтернатива.
  F12 в отдельном окне debugger не перехватывается главным окном.
- Полный reset затрагивает только `ZXMAK2.vmz`, `.vmide`, `.cmos`, `.nvram`;
  ROM, образы носителей, логи и пользовательские файлы не удаляются. Backups
  сохраняются в `StateBackups/<scope>-YYYYMMDD-HHMMSS` рядом с приложением.
- Release-сборка после этих изменений прошла без ошибок; остались два
  исторических предупреждения о отсутствующих `.ruleset`.
- Дополнительно выполнен штатный `Test.exe`: все ULA sanity-векторы и
  CPU/renderer benchmark-набор завершились успешно. Это проверка сборки;
  визуальная приёмка новых toolbar-иконок и hotkeys остаётся за пользователем.
- Иконки FDD и HDD уточнены как компактные 32×32 пиктограммы в ограниченной
  legacy-палитре (носитель/слот/LED), без фотореализма. SD остаётся штатной
  иконкой ZXMAK2. Зелёная/красная status-точка сохранена: размер 10 px,
  контрастная белая окантовка.
- После закрытия работающего `ZXMAK2.exe` обычная Release-папка
  `src/_binrelease` пересобрана успешно; новых предупреждений нет, остаются
  только два исторических `.ruleset`.
- По новому указанию пользователя старые медиа-пиктограммы заменены тремя
  32×32 PNG, полученными из предоставленного образца HDD/SD/FDD: фон удалён,
  палитра сведена к 18 общим legacy-цветам, прозрачность сохранена. Статусные
  точки остаются отдельным слоем поверх PNG, поэтому их размер и цвет не
  зависят от рисунка носителя.
- Для проверки этой правки собрана отдельная portable-папка
  `L:\Work_two\ZX\ZXMAK2-v14-ZXEVO-BC-MediaUI-20260922\release`; Alpha 5,
  её GitHub release и предыдущая B40 portable-папка не заменялись.
- После визуальной проверки 32×32 вариант признан слишком грубым: он
  уменьшал исходный рисунок и сводил его к слишком бедной палитре. Следующий
  вариант использует исходные цвета, прозрачный фон и отдельные полотна
  40×32 для HDD, SD и FDD; изображение носителя помещается целиком.
- Все выпадающие кнопки панели, включая выбор машины, получили одинаковую
  ширину 56×36: 40 px для рисунка и место справа для стрелки. Статусная точка
  остаётся отдельным контрастным слоем 10 px и не уменьшает носитель.
- Визуальная проверка показала, что 40×32 всё ещё не оставляло безопасного
  места для точки: она перекрывала правый край HDD и SD. Исправление: рабочие
  кнопки увеличены до 74×42, внутреннее полотно до 52×36; 12 px справа всегда
  свободны для точки. Рисунки получены заново из исходного JPEG в исходных
  цветах, с мягким сглаживанием и без принудительного сокращения палитры.
- Полная Release-сборка нового варианта прошла без ошибок; остались два
  старых предупреждения `.ruleset`. Для проверки создана отдельная portable
  папка `L:\Work_two\ZX\ZXMAK2-v16-ZXEVO-BC-MediaUI-FullArtwork-20260922\release`.
- По уточнённому требованию пользователя все три исходника (FDD/HDD/SD)
  подготовлены в двойном разрешении 104×72, затем основной UI мягко
  уменьшает их до 52×36. Правый 24-пиксельный запас в исходнике становится
  12-пиксельным полем для статусной точки; весь рисунок носителя остаётся
  слева от неё и не упирается в границу стрелки.
- Полная Release-сборка 2×-варианта завершена без ошибок (два прежних
  предупреждения `.ruleset`). Для неё создана отдельная portable-папка
  `L:\Work_two\ZX\ZXMAK2-v17-ZXEVO-BC-MediaUI-2xArtwork-20260922\release`.

## Сводка изменений после B40 до текущей UI-сборки — 2026-09-22

### Базовая контрольная точка

- B40 признана стабильной и опубликована как `ZX-Evo BaseConf Alpha 5 — B40`
  (`v0.5-alpha`). Тег исходников: `B40-stable`. Эта точка остаётся исходным
  безопасным откатом; post-B40 UI-работа ведётся в отдельной ветке
  `codex/post-alpha5-polish`.

### Лента BaseConf

- В чистый portable-профиль BaseConf добавлен уже существующий `TapeDevice`.
- TAP/TZX теперь воспроизводятся в правильной временной шкале: ×8 для
  `ZX-Evo BSconf` (master-clock 28 МГц), ×1 для остальных машин.
- Открытая лента сохраняется при переключении машин, но её скорость
  пересчитывается в момент воспроизведения. Поэтому переходы
  BaseConf→Pentagon и Pentagon→BaseConf не требуют повторного открытия
  TAP/TZX и не дают ускорения/замедления.

### Носители и их UI

- SD-карта получила постоянное хранение подключённого пути; после перезапуска
  она остаётся подключённой, как HDD/FDD, пока пользователь не сменит её или
  не сделает Eject.
- В Machine Settings добавлена полная страница SD: connect, выбор образа,
  read-only и Eject. Смена или извлечение SD использует тот же надёжный cold
  restart, что и HDD. FDD по умолчанию остаётся writable.
- SD/HDD/FDD переведены на единый host-слой команд. Панель и настройки не
  дублируют разную логику: SD/HDD сохраняют конфигурацию и выполняют cold
  restart при смене, FDD сохраняет конфигурацию без принудительного restart.
- На панели добавлены выпадающие кнопки FDD/HDD/SD: Load и Eject; зелёная
  точка означает подключённый носитель, красная — отсутствие. Диаметр точки
  `MediaStatusDotDiameter = 10` оставлен одной настраиваемой константой.

### Горячие клавиши и сохранённое состояние

- F12 — Warm Reset без подтверждения; Ctrl+F12 — подтверждённый reset CMOS;
  Ctrl+Alt+F12 — подтверждённый полный reset machine state. Alt+Ctrl+Insert
  сохранён, Alt+Ctrl+End добавлен как его альтернатива.
- F12 не перехватывается окном debugger, где у неё собственная функция.
- Перед CMOS/full reset создаётся timestamped backup в `StateBackups`; полный
  reset затрагивает только `.vmz`, `.vmide`, `.cmos`, `.nvram`, не ROM,
  носители, логи или пользовательские файлы.

### Текущий вид медиа-кнопок

- Все выпадающие кнопки, включая выбор машины, имеют равную ячейку 74×42.
- Для FDD/HDD/SD подготовлены новые рисунки по предоставленному образцу:
  исходники 104×72 мягко уменьшаются до 52×36. В правой части есть поле под
  статусную точку, поэтому ни HDD, ни SD, ни FDD не должны обрезаться справа.
- Предыдущие промежуточные варианты 32×32, 40×32 и 52×36 служили только
  этапами визуальной настройки; текущим вариантом является 104×72 → 52×36.

### Проверка и состав сборок

- Изменения после B40 в этом разделе не меняют BaseConf-порты, видео,
  звуковой тракт, тайминги, CPU, IDE/SD/FDD-контроллеры или содержимое Alpha
  5. Это host-интерфейс, профиль ленты и ресурсы UI.
- Штатный `Test.exe` после host-правок прошёл ULA sanity-векторы и
  CPU/renderer benchmark-набор. Все отдельные Release-сборки завершались с
  нулём ошибок; сохраняются лишь два прежних предупреждения `.ruleset`.
- Последняя отдельная тестовая portable-папка:
  `L:\Work_two\ZX\ZXMAK2-v17-ZXEVO-BC-MediaUI-2xArtwork-20260922\release`.
  Alpha 5 и её GitHub release не заменялись и не публиковались повторно.

## Коррекция SD и reset-индикаторов после UI-сборки — 2026-09-22

- После замечания пользователя обнаружено: сразу после выбора SD-образа
  Machine Settings иногда показывал пустое поле, хотя карта была подключена.
  Исправление сохраняет конфигурацию носителя до controlled cold restart и
  повторно после него; SD при сериализации также использует фактически
  подключённый путь как безопасный fallback.
- Статусные точки FDD/HDD/SD теперь принудительно перечитываются после F12
  Warm Reset и после успешного Ctrl+F12 reset CMOS. После Ctrl+Alt+F12
  приложение перезапускается с чистым machine state, поэтому его точки
  создаются заново по реальному состоянию носителей.
- По уточнённому решению пользователя timestamped backup перед Ctrl+F12 и
  Ctrl+Alt+F12 удалён. Подтверждения reset сокращены: CMOS возвращает
  настройки машины к исходным, полный reset возвращает ZXMAK2 к исходному
  состоянию. Образы носителей и ROM не удаляются.
- Эта коррекция ограничена host-настройками, SD-конфигурацией и UI; логика
  эмулируемой машины, контроллеры, видео, звук и тайминги не менялись.
- Полная Release-сборка прошла с 0 ошибок; остались только два известных
  предупреждения об отсутствующих `.ruleset`. Штатный `Test.exe` завершил
  все ULA sanity-векторы и CPU/renderer benchmark-набор успешно. Отдельная
  тестовая папка: `L:\Work_two\ZX\ZXMAK2-v18-ZXEVO-BC-ResetSdFix-20260922\release`.

## Точечная правка отображения SD в Machine Settings — 2026-09-22

- Пользователь подтвердил корректность точек FDD/HDD и их соответствия
  фактически подключённым образам. Оставшийся дефект был только у SD:
  зелёная точка показывала подключение, но Machine Settings открывал пустую
  временную копию профиля.
- При открытии Machine Settings временная SD-копия теперь получает путь
  непосредственно от реально смонтированной карты. Поэтому отмеченная
  галочка, путь и Eject доступны сразу после Load SD, даже для профиля,
  созданного предыдущей версией без сохранённого `sdImage`.
- Load/Eject SD также вызывают стандартное уведомление об изменении
  конфигурации, так что путь сохраняется в `ZXMAK2.vmz` сразу, а не только
  при следующем не связанном сохранении.
- FDD/HDD, status-точки, reset-логика и эмулируемая машина не менялись.
  Полная Release-сборка и штатный `Test.exe` успешно прошли; отдельная
  тестовая папка: `L:\Work_two\ZX\ZXMAK2-v19-ZXEVO-BC-SdSettingsFix-20260922\release`.

## Исправление Apply для уже подключённой SD — 2026-09-22

- Приёмка v19 показала второй участок той же ошибки: SD показывалась в
  Machine Settings, но нажатие Apply записывало старый пустой `sdImage` из
  временного bus-профиля и извлекало карту.
- Исправление синхронизирует путь реально подключённой SD не только с полем
  окна, но и с устройством в применяемой временной конфигурации. Поэтому
  Apply без изменения настроек сохраняет SD, а не извлекает её.
- Изменён только этот путь SD в Machine Settings. Полная Release-сборка и
  штатный `Test.exe` успешно прошли; отдельная тестовая папка:
  `L:\Work_two\ZX\ZXMAK2-v20-ZXEVO-BC-SdApplyFix-20260922\release`.

## SD в Machine Settings: окончательная схема без fallback-подпорок — 2026-09-22

- Проверка реализации HDD показала принципиальное отличие: окно Machine
  Settings создаёт временный sandbox-bus. IDE в нём хранит только путь к
  образу и не открывает файл второй раз; SD прежде пыталась открыть тот же
  образ с write-доступом параллельно с рабочей машиной. Это могло оставить
  видимым путь, но при Apply сбросить фактическое подключение.
- SD приведена к той же модели, что HDD: sandbox хранит только дескриптор
  `sdImage`; файл SD физически открывается только в живой машине, когда её
  конфигурация применяется. Поэтому `Load SD → Machine Settings → Apply`
  не должен извлекать уже подключённую карту.
- Удалены все промежуточные подпорки: подстановка пути из живого устройства,
  зеркалирование SD в `FormMachineSettings`, вспомогательное свойство
  смонтированного пути и специальные уведомления конфигурации из Load/Eject.
  FDD, HDD, точки статуса и эмуляция не менялись.
- Полная Release-сборка и штатный `Test.exe` успешно прошли. Отдельная
  тестовая папка: `L:\Work_two\ZX\ZXMAK2-v21-ZXEVO-BC-SdSandboxFix-20260922\release`.

## GitHub prerelease — ZX-Evo BaseConf Alpha 6 — B40 — 2026-09-22

- По результатам пользовательской runtime-приёмки медиа-UI и SD-схемы эта
  версия выпускается как portable prerelease `v0.6-alpha`. Эмуляция BaseConf
  не менялась: Alpha 6 содержит только host-интерфейс, profile-логику и
  согласованное управление носителями поверх принятой B40.
- В чистом portable-профиле default-машина переключена с Pentagon 128K на
  `ZX-Evo BSconf`. Носители, персональные состояния, логи и PDB в публичный
  архив не включаются; ZIP содержит одну корневую папку.

## No Border — отдельная проверочная сборка — 2026-09-22

- В `View → Scale Mode` добавлена независимая сохраняемая галочка `No Border`.
  Host-слой вырезает только аппаратный бордюр при показе кадра; выбранный
  Stretch/Keep Proportion/Fixed Pixel Size/Square Pixel Size применяется к
  оставшейся активной области.
- Активная область передаётся вместе с готовым кадром для Spectrum-совместимых,
  ATM/ZX-Evo wide/text и Profi режимов; полноэкранный режим использует ту же
  настройку. Эмуляция, тайминги и содержимое кадров не менялись.
- Отдельная сборка: `L:\Work_two\ZX\ZXMAK2-v23-ZXEVO-BC-NoBorder-20260922\release`.
  Alpha 6 и опубликованный prerelease не менялись.

## Коррекция benchmark Test.exe после смены default-машины — 2026-09-22

- Замедление `Test.exe` в Alpha 6 не оказалось регрессией CPU или видео:
  B21 и B22 имеют одинаковый IL всех тестовых, Engine и Hardware сборок.
  Перекрёстная проверка показала, что результат определяется только default
  профилем из `machines.config`: около 360 мс на 500 кадров с Pentagon и
  около 4,8 с после первоначального ZX-Evo профиля.
- Причина в harness: `Spectrum.Init()` сначала собирал default-машину, после
  чего `MainTest` загружался с повторным использованием совместимых устройств.
  У тестового профиля часть параметров намеренно не задана, поэтому он получал
  настройки предыдущей машины. Теперь временная default-конфигурация
  отключается и очищается до загрузки embedded `MainTest`.
- Исправлен только `src/Test/Program.cs`; холодный запуск, эмуляция и реальная
  смена машин не изменялись. Отдельно требуется решить, как при смене машины
  сочетать корректные defaults профиля с сохранением ленты и носителей.
- Полная Release-сборка Visual Studio MSBuild 17.14.51: 0 ошибок, два прежних
  предупреждения об отсутствующих ruleset. На ZX-Evo default `Test.exe` снова
  даёт 380–391 мс для 500 кадров `testVideo.z80` (B21-порядок), все ULA
  sanity-векторы проходят. Отдельная сборка: `L:\Work_two\ZX\ZXMAK2-v24-ZXEVO-BC-NoBorder-TestIsolation-20260922\release`.

## Уборка дубликата DirectX и стабильная привязка media-команд — 2026-09-22

- Удалён устаревший, не подключённый к `ZXMAK2.Host.WinForms.csproj` каталог
  `Mdx/Renderers.mdx`: в нём лежали четыре старые копии DirectX-рендереров,
  расходившиеся с фактически компилируемыми исходниками. Рабочий каталог
  `Mdx/Renderers` и код рендеринга не изменялись.
- Добавлен общий `IMediaCommand` с устойчивым типом носителя, действием
  Load/Eject и индексом FDD. Команды SD, HDD и FDD помечают себя этим
  контрактом, а `MainView` больше не определяет их назначение сравнением
  пользовательских текстов. Переименование или локализация надписей теперь
  не может незаметно отключить меню панели инструментов.
- Логика монтирования, извлечения, status-точек, reset, портов и эмулируемой
  машины не менялась. Полная Release-сборка Visual Studio MSBuild 17.14.51:
  0 ошибок; только два прежних предупреждения об отсутствующих `.ruleset`.
  Локальная чистая portable-тестовая сборка создана в
  `L:\Work_two\ZX\ZXMAK2-v25-ZXEVO-BC-MediaCommandCleanup-20260922\release`
  и ZIP рядом с ней. По уточнению пользователя в portable-папку также
  включены `Test.exe` и `Test.exe.config`; PDB всё так же исключены.
  GitHub-релиз не создавался.

## Иконка переключения машин: ресурс 52×36 — 2026-09-22

- Старая `EmuMachines_32x32` заменена одним новым прозрачным ресурсом
  `EmuMachines_52x36`. Изображение подготовлено из предоставленного
  пользователем образца с качественной бикубической интерполяцией, без
  растяжения исходной 32×32-иконки.
- Кнопка переключения машин уже имеет общую ширину 74×42; для неё явно
  отключено автоматическое масштабирование картинки. Поэтому рисунок
  занимает все 52×36 полезных пикселя слева от стрелки раскрытия, не обрезается
  и не пережимается обратно до 32×32.
- Старые PNG и SVG-версии ресурса удалены, дубликатов не оставлено. Логика
  выбора машины и эмуляция не менялись.
- Полная Release-сборка Visual Studio MSBuild 17.14.51: 0 ошибок, два прежних
  предупреждения об отсутствующих `.ruleset`. Новая чистая тестовая сборка:
  `L:\Work_two\ZX\ZXMAK2-v26-ZXEVO-BC-MachineIcon-20260922\release`; ZIP
  находится рядом. В неё включён `Test.exe`, но не включены PDB и личные
  состояния.

## Иконка переключения машин: финальный визуальный вариант — 2026-09-22

- Пользователь отклонил первый вариант v26 как слишком мелкий и перегруженный.
  Он сохранён только как отдельная прежняя тестовая папка и не является
  продолжением для проверки.
- Ресурс `EmuMachines_52x36` заменён на цельный мотив из последнего
  предоставленного образца: две машины и круговая стрелка переключения.
  Изображение вырезано целиком, сохранены пропорции, подготовлен прозрачный
  фон и применена качественная бикубическая интерполяция. Оно не обрезается
  областью стрелки ToolStrip и не масштабируется обратно до 32×32.
- Полная Release-сборка: 0 ошибок, два прежних предупреждения `.ruleset`.
  Чистая portable-проверочная версия:
  `L:\Work_two\ZX\ZXMAK2-v27-ZXEVO-BC-MachineIcon-Refresh-20260922\release`
  и ZIP рядом; `Test.exe` включён, PDB и личные состояния исключены.

## GitHub prerelease — ZX-Evo BaseConf Alpha 7 — B40 — 2026-09-22

- Выпускается `v0.7-alpha` как portable prerelease поверх принятой B40. В ZIP
  одна корневая папка, `Test.exe` и `Test.exe.config` включены; личные `.cmos`,
  `.vmide`/`.vide`, `.vmz`, PDB, образы носителей и логи исключены.
- В Alpha 6 обнаружено замедление диагностического `Test.exe`: стартовая
  конфигурация `ZX-Evo` наследовалась embedded тестовым профилем. Это не было
  замедлением CPU или renderer. Alpha 7 очищает временный default до загрузки
  `MainTest`; штатный benchmark снова даёт 339–397 мс на 500 кадров и все ULA
  sanity-векторы проходят.
- В состав также входят No Border во всех масштабных режимах и Full Screen,
  стабильные media-команды панели и обновлённый ресурс выбора машины. Ядро
  BaseConf B40, лента, FDD/HDD/SD, звук, видео и тайминги не менялись.

## Tape Eject и горячая клавиша No Border — 2026-09-22

- Предыдущая попытка с `Ctrl+B` отменена отдельным коммитом: эта комбинация
  успевала попасть в виртуальную клавиатуру BaseConf и могла вызывать команду
  загрузчика. Комбинации `Alt+F9` и `Ctrl+Alt+F11` не срабатывали с физической
  клавиатуры. Для `No Border` используется одиночная `F11`, назначенная штатным
  свойством WinForms `ToolStripMenuItem.ShortcutKeys`, а не строкой
  `ShortcutKeyDisplayString`: последняя отвечает только за надпись в меню.
  Штатный обработчик меню вызывает тот же `Click`, что и нажатие мышью, поэтому
  виртуальная клавиатура ZX Evo эту F11 не получает.
- В плеере ленты добавлена кнопка `Eject`. Она вызывает операцию самого
  устройства ленты: останавливает воспроизведение, сбрасывает позицию,
  гасит индикатор и очищает блоки. Форма плеера очищает свой список и
  отключает кнопки, а главное окно получает отдельное событие и снимает
  имя `.tap`/`.tzx` из заголовка.
- Привязка события пересоздаётся при смене машины и корректно отписывается
  при завершении главной модели; никаких изменений в логике чтения ленты,
  портов, таймингов или в самой эмулируемой машине нет.
- Проверка открытия `cpd-test.tap` в отдельной локальной копии: заголовок
  меняется на `[cpd-test.tap] - ZXMAK2`, а Tape показывает четыре блока.
  Полная Release-сборка Visual Studio MSBuild 17.14.51: 0 ошибок и два
  прежних предупреждения об отсутствующих `.ruleset`. `Test.exe`: все ULA
  sanity-векторы прошли; benchmark 500 кадров — 343–412 мс (основные
  профили), 206–220 мс (CPU).

## Кандидат в ZX-Evo BaseConf Alpha 8 — изменения после Alpha 7 — 2026-09-22

Статус: **локальная проверочная сборка; не опубликована и не является GitHub
релизом.** Пользователь проведёт расширенную runtime-проверку 2026-09-23. Если
она завершится успешно, Alpha 8 будет собрана как чистый portable-пакет и
опубликована только по отдельной команде пользователя.

Возможный состав Alpha 8 относительно опубликованной Alpha 7:

1. **BaseConf CD/DVD как IDE Slave (B41-R1).** В `Machine Settings → PentEvo
   IDE Settings` добавлена отключённая по умолчанию read-only опция физического
   или смонтированного Windows CD/DVD-привода. Реализованы документированные
   ATAPI-команды, нужные для загрузчика Time Gal; `Eject` отключает привод от
   эмулятора и не открывает физический лоток. Основная эмуляция B40, IDE Master
   и ATM Turbo не изменялись. Автоматические `AtapiCdromProbe-B41` дают 25 PASS
   на `timegal.iso`; пользователь подтвердил запуск Time Gal. Воспроизведение
   NedoVIDEO/ATM Turbo в этот состав **не принято** и остаётся отдельной
   задачей, а не заявленной функцией Alpha 8.
2. **Tape Eject.** В форме Tape справа от управления воспроизведением добавлена
   кнопка `Eject`. Она действительно выгружает открытый TAP/TZX, очищает список
   блоков и имя файла в заголовке, останавливает ленту и гасит индикатор.
3. **Горячая клавиша No Border.** Штатная WinForms-привязка `F11` установлена
   непосредственно у пункта `View → Scale Mode → No Border` через
   `ToolStripMenuItem.ShortcutKeys`; нажатие выполняет ровно тот же обработчик,
   что и щелчок мышью. Прежние нерабочие экспериментальные комбинации в
   публичный состав не входят.

Финальная локальная Release-сборка после этих правок: 0 ошибок, только два
старых предупреждения о отсутствующих ruleset. `Test.exe` прошёл все ULA
sanity-векторы; renderer benchmark — 361–406 мс за 500 кадров, CPU — 206–210
мс. Коммит состояния кандидата: `eecb936`.

Для пользовательской проверки создана чистая папка
`L:\Work_two\ZX\ZXMAK2-v30-ZXEVO-BC-Alpha8-Candidate-20260922\release`:
48 файлов, 8.41 МБ. В ней есть `Test.exe`, но нет PDB, логов, `.cmos`,
`.vmide`/`.vide`, `.vmz`, образов носителей или старых ZIP. SHA-256
`ZXMAK2.exe`: `EDFB4AF00C7055C1D724D99E5111857017FE1D5D1B456D7380985A1BB2BCC643`.

## PENTEVO Internal Sound и основа двух ZXBUS-слотов — v34 — 2026-09-23

Статус: **отдельная локальная тестовая сборка; не Alpha 8 и не GitHub-релиз.**
Работа начата точно от зафиксированной основы кандидата Alpha 8 v30
(`61cef7b`). Папка v30 не изменялась; её контрольный SHA-256
`ZXMAK2.exe` после работы остался
`EDFB4AF00C7055C1D724D99E5111857017FE1D5D1B456D7380985A1BB2BCC643`.

- По официальной документации NedoPC у ZX Evolution два физических слота
  ZXBUS на общей адресной, данной и управляющей шине. Поэтому в PENTEVO
  добавлены два логических места `Slot 1` и `Slot 2`, а не две
  независимые выдуманные шины. Состояния активности и выбранной платы
  сохраняются отдельно для каждого слота и проходят XML round-trip.
- На текущем этапе единственный вариант платы — `Empty`. Неудачные
  эксперименты TurboSound FM Pro/SAA1099 из v31-v33 в v34 не переносились;
  NeoGS тоже не добавлялась. Реальные варианты появятся только после
  отдельной законченной эмуляции соответствующей платы.
- В `Machine Settings → PENTEVO` добавлен единый выбор
  `Internal sound`: `AY/YM (AY8910-CHRV)` либо `None`, плюс громкость
  AY/YM. Выбор действительно добавляет или удаляет внутренний AY из
  исполняемой конфигурации. Чтобы нельзя было создать конфликт, этот
  внутренний AY скрыт из общего списка машины и исключён из мастера
  добавления устройств. Beeper и Covox остаются отдельными штатными
  выходами BaseConf и не менялись.
- Исправлена одна очевидная локальная ошибка: мастер добавления устройств
  больше не падает, если список уже установленных устройств не задан.
  Эмуляция CPU, видео, таймингов, FDD/HDD/SD/CD, ленты и reset не менялась.
- Полная Release-сборка Visual Studio MSBuild 17.14.51: 0 ошибок, только два
  прежних предупреждения об отсутствующих `.ruleset`. Новый
  `PentEvoProfileProbe` проверяет переключение AY↔None, громкость, оба
  слота, сохранение/загрузку XML, точную привязку панели PENTEVO и отсутствие
  обрезанных элементов. Все проверки прошли. Скрытый startup smoke-test
  ZXMAK2 прошёл.
- `Test.exe`: все ULA sanity-векторы прошли; renderer 351–405 мс за
  500 кадров, light 265–270 мс, CPU 261–266 мс.
- Чистая portable-папка:
  `L:\Work_two\ZX\ZXMAK2-v34-ZXEVO-BC-Alpha8-ZXBUS-Foundation-20260923\release`.
  В ней 48 файлов (8 820 481 байт), включая `Test.exe`; отсутствуют PDB,
  probe-файлы, логи, образы носителей и пользовательские `.cmos`,
  `.nvram`, `.vmide`/`.vide`, `.vmz`.
  SHA-256 `ZXMAK2.exe`:
  `A2EEB47D7FF37454DC471A31E3AAE270266E0375ED1BFC8AC0C538D3B40272C0`.

## NeoGS Rev. C-VS в ZXBUS — v35 — 2026-09-23

Статус: **отдельная локальная тестовая сборка; не Alpha 8 и не GitHub-релиз.**
Работа продолжает принятую основу v34, а v30 и v34 не изменялись. Их
контрольные SHA-256 `ZXMAK2.exe` после работы соответственно:
`EDFB4AF00C7055C1D724D99E5111857017FE1D5D1B456D7380985A1BB2BCC643` и
`A2EEB47D7FF37454DC471A31E3AAE270266E0375ED1BFC8AC0C538D3B40272C0`.

- Основа сверена с текущей страницей NedoPC
  `http://nedopc.com/gs/ngs.php` и официальным FPGA-проектом
  `https://github.com/tslabs/neogs` (`a1c3bbcd`). Выбрана последняя указанная
  аппаратная ревизия **NeoGS C-VS**. В сборку встроена официальная полная
  прошивка **1.11 от 07.02.2026**, размер 524288 байт, SHA-256
  `764FC819F831F64CE736D9254AF4FB6DD6FCD154D09F494F3AFAB92543A0CD37`.
- Плата реализована новым синхронным устройством без прежнего фонового потока:
  собственный Z80, 4 МБ RAM, 512 КБ ROM, документированные порты ZXBUS
  `#BB/#B3/#33`, внутренние порты `#00-#23`, обычный и расширенный paging,
  прямые регистры четырёх страниц, частоты 10/12/20/24 МГц, таймер и
  контроллер прерываний. Векторы timer/SD/MP3 соответствуют текущей FPGA:
  `#FF/#F7/#EF`.
- Реализован основной звуковой тракт NeoGS: частота DAC 37,5 кГц, режимы
  4/8 каналов, восемь шестибитных громкостей, `PAN4CH` и `INV7B`. Работа
  платы синхронизирована с тактами основной машины, поэтому обмен командами
  через ZXBUS детерминирован и не зависит от потока Windows.
- В `Machine Settings → PENTEVO → ZXBUS expansion slots` NeoGS можно поставить
  в Slot 1 либо Slot 2. Одна физическая плата не может одновременно занимать
  оба слота. Выбор реально добавляет/удаляет устройство, сохраняется в XML и
  не дублируется в общем мастере устройств.
- Панель PENTEVO заново уложена в её фактическую рабочую ширину 284 пикселя:
  выбор внутреннего AY/YM, громкость и оба ZXBUS-слота полностью помещаются;
  прежняя обрезка правой стороны устранена.
- Граница текущего этапа записана явно: встроенные в C-VS интерфейсы microSD
  и VS10xx/MP3 пока представлены документированными регистрами без
  подключённого виртуального носителя и без декодирования MP3. Это не мешает
  обычному GS/NeoGS обмену и 4/8-канальному воспроизведению, но эти две
  дополнительные функции нельзя заявлять завершёнными до отдельного этапа.
- Проверки: официальная ROM запускается и сама переключает Z80 платы на
  документированные 20 МГц; команда хоста принимается и подтверждается;
  проверены paging, RAM, порты, громкости, векторы прерываний, XML и звуковой
  mixer. Прошли `PentEvoProfileProbe`, полный `Test.exe`, а также актуальные
  B39A/B40 regression-тесты палитры, семи renderer-режимов, mid-frame,
  contention и floating bus. Полная Release-сборка: 0 ошибок, два прежних
  предупреждения об отсутствующих `.ruleset`.
- Чистая portable-папка:
  `L:\Work_two\ZX\ZXMAK2-v35-ZXEVO-BC-Alpha8-NeoGS-Test-20260923\release`.
  В ней 48 файлов (9351937 байт), включая `Test.exe`; PDB, probes, логи,
  пользовательские состояния и образы носителей исключены. SHA-256:
  `ZXMAK2.exe` —
  `FC3DB5FA59FF7876B7BE6803D7FC9DF8894C1D2EBEE83BD1E9F26B4152C1EF61`,
  `ZXMAK2.Hardware.dll` —
  `4034FEC8AFEFBB498BB6843A5F599AA9EDAB550E550B52B890A9A3C634AEA890`.

## NeoGS: выходной уровень 150% — v36 — 2026-09-23

- После успешной пользовательской проверки Z-PLAYER v4.1 и обычного
  четырёхканального MOD основной тракт NeoGS признан работающим.
- Только для выхода NeoGS добавлен фиксированный коэффициент 1,5. Положительные
  и отрицательные значения масштабируются симметрично, а итог ограничивается
  допустимым 16-битным диапазоном, поэтому переполнение не превращается в
  цифровой треск. Порты, тайминги, прошивка и остальные источники звука не
  изменялись.
- Полная Release-сборка, NeoGS probe и штатный `Test.exe` прошли. Отдельная
  чистая папка:
  `L:\Work_two\ZX\ZXMAK2-v36-ZXEVO-BC-Alpha8-NeoGS-Gain150-Test-20260923\release`.
  48 файлов, 9352449 байт. SHA-256 `ZXMAK2.exe`:
  `837AD07D818D1B13447E33C0779C900E87922AEF1B1847F901973F25A1EB7816`;
  SHA-256 `ZXMAK2.Hardware.dll`:
  `A79C5FBB2D6D7024604C91D15A6547DBE3CB6CE6F7C9B471C0C2703499E07EA4`.
  v35 сохранена как предыдущая версия с исходным уровнем звука.

## NeoGS C-VS: microSD, DMA и VS1011/MP3 — v37 — 2026-09-23

Статус: **отдельная локальная тестовая сборка; не Alpha 8 и не GitHub-релиз.**
Работа продолжает принятую v36. Папки v30, v34, v35 и v36 не изменялись и
остаются точками быстрого отката.

- Дополнительные узлы NeoGS C-VS заново сверены с официальными исходниками
  FPGA `tslabs/neogs` (`dma_zx.v`, `dma_sd.v`, `dma_mp3.v`, `ports.v`) и
  документацией `dma_zx_doc.txt`/`spi_doc.txt`. Для проверки поведения также
  использованы исходники Unreal Speccy из указанной пользователем ревизии SVN
  r1365 (`gsz80.cpp`, `sdcard.cpp`, `vs1001.cpp`); они служили справочным
  материалом и в поставку не входят.
- Реализована виртуальная microSD NeoGS: подключение raw IMG/IMA/VHD, SPI-команды
  карты, сохранение пути в конфигурации, безопасное отключение и отдельные
  органы управления на странице PENTEVO. Панель увеличена по высоте, а
  элементы уложены в прежнюю рабочую ширину без обрезки справа.
- Реализованы три документированных DMA-канала: ZX↔RAM платы, SD→RAM и
  RAM→VS1011. Переносы обновляют адреса и состояние контроллера, соблюдают DREQ
  VS1011 и выставляют документированные IRQ-векторы SD/MP3.
- Добавлена модель VS1011: SCI-регистры и reset, SDI/DREQ, громкость и потоковое
  декодирование MPEG Layer I/II/III. Для декодирования используется NLayer
  1.11.0; `NLayer.dll` и её MIT-лицензия включены в portable-пакет. Реальный
  потоковый тест выявил и устранил ошибку первого варианта, где декодер ждал
  конца всего потока вместо начала воспроизведения поступающих кадров.
- Автотест NeoGS теперь проверяет официальную ROM 1.11, paging/регистры,
  VS1011 SCI, живое декодирование MPEG-1 Layer III, созданный тестом FAT-образ,
  CMD17, 512-байтный SD DMA, 512-байтный MP3 DMA, IRQ и XML-конфигурацию.
  Результат: `PASS: NeoGS Rev. C-VS core, ROM 1.11, DMA, microSD and VS1011`.
  Профильный тест PENTEVO также прошёл.
- Полная Release-пересборка: 0 ошибок, только два прежних предупреждения об
  отсутствующих `.ruleset`. `Test.exe` из готовой v37 прошёл все ULA sanity;
  renderer 332–396 мс за 500 кадров, light 263–264 мс, CPU 231–242 мс.
  Скрытый startup smoke-test готового `ZXMAK2.exe` прошёл.
- Чистая portable-папка:
  `L:\Work_two\ZX\ZXMAK2-v37-ZXEVO-BC-Alpha8-NeoGS-Full-Test-20260923\release`.
  В ней 50 файлов (9438024 байта), включая `Test.exe`, `NLayer.dll` и
  `NLayer.LICENSE.txt`; нет PDB, probes, логов, образов и пользовательских
  `.cmos`, `.nvram`, `.vmide`/`.vide`, `.vmz`. Стартовый профиль — чистая
  `ZX-Evo BSconf`, оба слота ZXBUS выключены. SHA-256:
  `ZXMAK2.exe` —
  `903794B161E54215906C6D605092DDE0F7F7A3CBF442B2236CDEF7508626F5DF`;
  `ZXMAK2.Hardware.dll` —
  `0780FCBADE8E9233A50E8FF71711341C93C1E080779CBB50A5A9C65B6E3448BD`;
  `ZXMAK2.Host.WinForms.dll` —
  `A9C83AE318A30A95DFF93CFEE7992CE17422DDAD9BDE9BE2980B7D9B44F964FD`;
  `NLayer.dll` —
  `FFD3EF961B3707A963ED40287FEAA835CEFEEC4A3F7C923DE7BBAB71BE38B0A2`.

## NeoGS media UI, FDD-индикаторы и уровень MP3 — v38 — 2026-09-24

Статус: **отдельная локальная тестовая сборка; не Alpha 8 и не
GitHub-релиз.** Работа продолжает принятую пользователем v37; папка v37 не
изменялась и остаётся точкой быстрого отката. Приёмка v37 подтвердила, что
NeoDOS определяет NeoGS и воспроизводит как MP3, так и MOD. Отдельный старый
MP3-плеер с платой не заработал, но это не воспроизводится в NeoDOS и не
считается отказом эмуляции NeoGS.

- В списке `Machine Settings` теперь явно разделены два независимых носителя:
  `SD Z-controller` — штатная SD BaseConf и `SD NeoGS` — microSD самой платы
  NeoGS. Вторая строка расположена сразу после первой, доступна только когда
  NeoGS действительно включена в одном из двух слотов ZXBUS, и меняет тот же
  экземпляр платы без создания дубликата устройства. Изменение слотов в ещё
  не закрытом окне сразу обновляет доступность строки.
- В выпадающем меню FDD слева от каждой команды `Eject A`…`Eject D` добавлен
  собственный индикатор: зелёный при вставленной дискете и красный при пустом
  дисководе. Состояние обновляется существующим 100-мс таймером панели и
  опрашивается через общий интерфейс Beta Disk; поддержаны обычный и
  Sprinter-контроллеры. Изображения индикаторов освобождаются при перестройке
  меню, поэтому повторное создание меню не накапливает GDI-ресурсы.
- Только декодированный поток MP3 NeoGS усилен ровно на 40% с симметричным
  насыщением 16-битного результата. Общий NeoGS DAC/MOD остаётся на принятом
  уровне 150%, остальные звуковые устройства не менялись.
- Полная Release-пересборка: 0 ошибок, два прежних предупреждения об
  отсутствующих `.ruleset`. `NeoGsProbe` проверил точный коэффициент MP3,
  ROM/DMA/microSD/VS1011; `PentEvoProfileProbe` — оба пункта SD, зависимость
  от слота, порядок навигации, состояние FDD и цвета точек. Оба теста прошли.
- Штатный `Test.exe` готовой папки прошёл все ULA sanity-векторы: renderer
  330–399 мс за 500 кадров, light 253–254 мс, CPU 244–255 мс. Для сохранения
  принятой скорости пакет собран консервативно от v37 с заменой только четырёх
  реально изменившихся DLL; полная пересборка неизменённых старых проектов
  используется для проверки компиляции, но не подменяет проверенный launcher
  и test harness. Скрытый startup smoke-test готового `ZXMAK2.exe` прошёл.
- Чистая portable-папка:
  `L:\Work_two\ZX\ZXMAK2-v38-ZXEVO-BC-Alpha8-NeoGS-UI-FDD-MP3-Test-20260923\release`.
  В ней 50 файлов (9442120 байт), включая `Test.exe`, `NLayer.dll` и лицензию;
  нет PDB, probes, логов, образов и пользовательских `.cmos`, `.nvram`,
  `.vmide`/`.vide`, `.vmz`. Стартовый профиль остаётся чистой
  `ZX-Evo BSconf`. SHA-256:
  `ZXMAK2.exe` —
  `903794B161E54215906C6D605092DDE0F7F7A3CBF442B2236CDEF7508626F5DF`;
  `ZXMAK2.Engine.dll` —
  `F944B8146F4B8131809EB8922600845F91FCEAA5F4CA33324BB3CBD91F4131FF`;
  `ZXMAK2.Hardware.dll` —
  `87F79845D0409DE66FDC78F1E72F23E92D8418E50A785755D9C69D873B9B615B`;
  `ZXMAK2.Host.Presentation.dll` —
  `EF1DB2645159DEA15BFB8FE6A55AF81D209F1EFB2F44909B856778C7AA231A06`;
  `ZXMAK2.Host.WinForms.dll` —
  `CA27017D82D0D0AE8FA33203E6721893CC2627CCF9A8CFFE5FED6F2EBEEEAD65`;
  `NLayer.dll` —
  `FFD3EF961B3707A963ED40287FEAA835CEFEEC4A3F7C923DE7BBAB71BE38B0A2`.

## Раздельное меню двух SD-карт — v39 — 2026-09-24

Статус: **отдельная локальная тестовая сборка; не Alpha 8 и не
GitHub-релиз.** Основа — v38; папка v38 не изменялась. Перед этим
пользовательская проверка подтвердила одновременную работу трёх носителей в
браузере BaseConf: IDE/HDD, microSD NeoGS и SD Z-controller. Для доступа к
microSD NeoGS в Setup браузера должна быть включена штатная опция доступа к
NeoGS; отдельная ошибка эмуляции носителя не обнаружена.

- Выпадающее меню общей кнопки SD теперь содержит короткие отдельные команды
  `Load Z-controller`, `Eject Z-controller`, `Load NeoGS`, `Eject NeoGS`.
  Команды NeoGS регистрируются самой установленной в ZXBUS платой и поэтому
  отсутствуют, когда платы нет в машине.
- У каждой строки Eject есть такой же десятипиксельный индикатор, как у FDD:
  зелёный — соответствующая карта смонтирована, красный — карта не
  смонтирована. Z-controller и NeoGS имеют независимые стабильные индексы 0
  и 1; статус обновляется существующим 100-мс таймером панели.
- Общая точка возле иконки SD зелёная, когда вставлена хотя бы одна из двух
  карт, и красная, когда обе пусты. Загрузка и извлечение NeoGS microSD через
  панель сохраняют конфигурацию и выполняют тот же контролируемый power cycle,
  что уже используется для штатной SD.
- Заодно исправлено освобождение картинок индикаторов при полной перестройке
  меню: ссылки больше не очищаются до их `Dispose`.
- Полная Release-пересборка: 0 ошибок, два прежних предупреждения `.ruleset`.
  `NeoGsProbe` и `PentEvoProfileProbe` прошли, включая настоящее монтирование
  microSD и раздельные индексы обеих карт. `Test.exe` прошёл все ULA sanity;
  renderer 327–403 мс, light 254–259 мс, CPU 234–249 мс. Startup smoke-test
  прошёл. Ядро Engine в этом этапе исходно не менялось, поэтому portable-пакет
  сохраняет уже проверенную DLL Engine из v38; заменены только действительно
  затронутые Host/Hardware/Presentation/WinForms-компоненты.
- Чистая portable-папка:
  `L:\Work_two\ZX\ZXMAK2-v39-ZXEVO-BC-Alpha8-Dual-SD-Indicators-Test-20260924\release`.
  50 файлов, 9445192 байта; пользовательские состояния, образы, PDB, probes и
  логи отсутствуют. SHA-256:
  `ZXMAK2.exe` —
  `903794B161E54215906C6D605092DDE0F7F7A3CBF442B2236CDEF7508626F5DF`;
  `ZXMAK2.Engine.dll` —
  `F944B8146F4B8131809EB8922600845F91FCEAA5F4CA33324BB3CBD91F4131FF`;
  `ZXMAK2.Host.dll` —
  `6F550A222B7B3A605262FACE27FEFEF4FDC13EBC0A5927BE4407C4C2079F9AE3`;
  `ZXMAK2.Hardware.dll` —
  `90630B8B8623CBA9011DE99BB9DED48A06A3051CE719557370DE8269A7B90AD9`;
  `ZXMAK2.Host.Presentation.dll` —
  `5AF4C521CF835E02AB3A15F0B2C2A68E7BE036BF92E4370F9C588620FE218367`;
  `ZXMAK2.Host.WinForms.dll` —
  `E8544C70C2742C81918DD618ECE7F0C9C618B60DB523C1E68890AD39F630F3AD`.

## TurboSound FM Pro Rev. C — v40 — 2026-09-24

Статус: **отдельная локальная тестовая сборка; не Alpha 8 и не
GitHub-релиз.** Основа — принятая пользователем v39; её папка не изменялась.
Неудачные реализации TSFM из старых v31–v33 не переносились.

- Сверены официальные исходники NedoPC `tsfmpro/trunk` SVN r576, включая
  документацию Rev. C и исполняемую логику CPLD `cfg.v`, `bus.v`, `clocks.v`.
  Для проверки программной совместимости отдельно изучена реализация
  Unreal Speccy `pentevo/tools/unreal_fix/0.39.0/Unreal_NS` SVN r1365.
- Исправлена принципиальная ошибка прежнего прототипа: SAA1099 платы TSFM Pro
  не использует отдельные порты `#1FF/#FF`. После выбора конфигурацией
  `%1111xxxx` он получает адрес и данные через обычную пару AY/YM
  `#FFFD/#BFFD`. Все 16 управляющих значений `#F0..#FF` распознаются CPLD,
  отдельно соблюдены выбор двух YM2203, разрешение FM, выбор register/status
  read и reset-состояние `1111`.
- Добавлена новая чистая составная модель платы: два независимых SSG YM2203,
  SAA1099 с шестью tone/noise-каналами и двумя envelope-генераторами при
  документированных 8 МГц, а также двухчиповая четырёхоператорная FM-часть
  YM2203 с алгоритмами, feedback, key-on, множителями, TL и ADSR. SSG работает
  от внутренней половины документированного такта YM2203 3,5 МГц.
- Плата устанавливается там же, где физически устанавливается оригинал:
  `Machine Settings → PENTEVO → Internal sound → TSFM Pro Rev. C`. Это
  взаимоисключающий вариант штатного AY/YM, а не устройство ZXBUS. NeoGS,
  оба слота ZXBUS и обе SD-карты не менялись. Порядок прежних пунктов AY/YM и
  None сохранён для совместимости существующей настройки и автотеста.
- Новый тест `Test.exe /tsfm` выполняет обращения настоящими инструкциями Z80
  через порты платы. Результат: D1 `#20`, D2 `#40`, status `#00`; все четыре
  источника D1/D2/FM/SAA дали ненулевой звуковой кадр — PASS.
- Полная Release-пересборка: 0 ошибок, два прежних предупреждения `.ruleset`.
  Штатный `Test.exe` прошёл все ULA sanity; renderer 324–393 мс, light
  249–254 мс, CPU 240–260 мс. Повторно прошли NeoGS, PENTEVO profile,
  Palette444 (132867), seven-renderer, mid-frame и contention/floating-bus.
  Startup smoke-test готовой папки прошёл.
- Чистая portable-папка:
  `L:\Work_two\ZX\ZXMAK2-v40-ZXEVO-BC-Alpha8-TSFMPro-RevC-Test-20260924\release`.
  50 файлов, 9460552 байта; нет PDB, probes, логов, образов и пользовательских
  `.cmos`, `.nvram`, `.vmide`/`.vide`, `.vmz`. Стартовая машина остаётся
  чистой `ZX-Evo BSconf`, TSFM по умолчанию не включена. SHA-256:
  `ZXMAK2.exe` —
  `903794B161E54215906C6D605092DDE0F7F7A3CBF442B2236CDEF7508626F5DF`;
  `ZXMAK2.Engine.dll` —
  `D5E734DE4A7FCEBA549CDA202F3EC4FEE620D213DA55FF0F575FB55678D87411`;
  `ZXMAK2.Hardware.dll` —
  `4DE78E27A09553FA818EE889F67AF9F8F29F4E0AFFF97C3818B8C2E2741EB3F1`;
  `ZXMAK2.Host.WinForms.dll` —
  `F74C4DF0614E85C3B96CE0AF7A987C455D577BD466DC42C3DFDE1885773B3029`;
  `Test.exe` —
  `18B04BDAB702F417BA8A40C1BD2ED9DD3260A4BFBD1B03AE272F000649FBAD83`.

## Integrated media controls and TSFM gain — v41 — 2026-09-24

Статус: **отдельная локальная тестовая сборка; кандидат на продолжение Alpha 8,
но ещё не GitHub-релиз.** Основа — принятая пользователем v40; её portable-папка
не изменялась. Контрольный SHA-256 `ZXMAK2.exe` в v40 после работ остался
`903794B161E54215906C6D605092DDE0F7F7A3CBF442B2236CDEF7508626F5DF`.

- Громкость всех источников TurboSound FM Pro Rev. C поднята ровно на 50%:
  оба SSG, FM и SAA1099. Усиление выполняется после сведения каждого источника
  с насыщением до диапазона signed 16-bit, поэтому переполнение и заворот
  сигнала исключены. `Test.exe /tsfm` теперь отдельно проверяет коэффициент
  усиления и насыщение.
- TSFM перенесён из специальной страницы PentEvo в обычный список устройств
  `Music`. Он предлагается только машинам, где уже доступен AY/YM-совместимый
  музыкальный тракт, и взаимоисключаем со штатным AY/YM. NeoGS остаётся
  самостоятельной ZXBUS-платой и может работать одновременно с выбранным
  внутренним музыкальным устройством.
- Для `ZX-Evo BSconf` на странице `Music` добавлен прямой выбор между
  `AY/YM (AY8910-CHRV)` и `TurboSound FM Pro Rev. C`. При замене сохраняются
  громкость и позиция устройства в шине; другие профили машин продолжают
  пользоваться прежним механизмом добавления/удаления музыкальных устройств.
- Для PentEvo восстановлена стандартная страница настройки ULA. Настройки двух
  физических слотов вынесены в отдельную страницу `ZXBUS PentEvo`; изменение
  ULA больше не стирает состояние слотов. Страница NeoGS microSD доступна только
  при реально установленной NeoGS. Обе SD-карты остаются раздельными носителями.
- На панель добавлена отдельная кассета. Основной щелчок открывает штатный
  магнитофон; меню содержит Load, Eject, Play, Stop, Rewind, Quick Load,
  Auto Play и Tape Player и использует те же команды и настройки, что прежний
  интерфейс. Состояние отображается единообразно: серые иконка и индикатор —
  устройство недоступно, цветная и красный индикатор — доступно, но носителя
  нет, цветная и зелёный индикатор — лента вставлена.
- Крайним справа добавлен CD/DVD. Основной щелчок подключает доступный
  Windows-привод как IDE slave либо предлагает выбор; меню содержит Connect,
  Eject и Settings. Пункт `CD-ROM Settings...` открывает `Machine Settings`
  сразу на странице `IDE PentEvo`, а не на общей первой странице. Eject
  полностью отключает и очищает привод. Обычный reset сохраняет подключение,
  factory reset его сбрасывает. Индикация использует те же три состояния:
  серый — устройство/привод недоступен, красный — подключён, но диск не готов,
  зелёный — диск готов к чтению.
- Для кассеты и компакт-диска подготовлены чистые прозрачные toolbar-ресурсы
  `EmuTapeImage_104x72.png` и `EmuCdImage_104x72.png`. Изображения вписаны
  целиком, с запасом под стрелку выпадающего меню; уменьшение выполняется с
  качественной интерполяцией, без обрезки исходных объектов.
- Унифицированы media-команды и трёхсоставный статус для ленты, оптического
  диска, FDD и обеих SD-карт. Недоступный пункт остаётся видимым, но становится
  серым и неактивным; пустой доступный носитель отмечается красным, вставленный
  — зелёным.
- После обнаруженного первого запуска в схлопнутом окне добавлена точечная
  защита сохранённой геометрии. Если ширина меньше 256 либо высота меньше 192,
  повреждённая пара заменяется на штатные 640×512 и сразу исправляется в
  `HKCU\SOFTWARE\ZXMAK2`. Проверено отдельно: 120×0 → 640×512, а нормальное
  800×600 остаётся без изменений.
- Полная Release-пересборка: 0 ошибок, только два прежних предупреждения о
  отсутствующих `.ruleset`. `Test.exe /tsfm` прошёл D1/D2/FM/SAA и новую
  проверку +50%/saturation. `NeoGsProbe` прошёл ROM 1.11, DMA, microSD и VS1011.
  `PentEvoProfileProbe` прошёл стандартную ULA, отдельные Music/ZXBUS,
  round-trip настроек, замену AY→TSFM→AY с сохранением громкости/порядка,
  переход CD-ROM Settings на `IDE PentEvo` и расположение/состояния новых
  toolbar-кнопок.
  `AtapiCdromProbe-B41 N:\` — 16 PASS на настоящем пустом Windows-приводе.
  Startup smoke-test прошёл. Полный `Test.exe`: все ULA sanity — PASS;
  renderer 380–410 мс, light 269–274 мс, CPU 239–241 мс.
- Чистая portable-папка:
  `L:\Work_two\ZX\ZXMAK2-v41-ZXEVO-BC-Alpha8-Integrated-Media-TSFM-Test-20260924\release`.
  51 файл, 9507302 байта; отсутствуют PDB, probes, логи, образы и пользовательские
  `.cmos`, `.nvram`, `.vmide`/`.vide`, `.vmz`. Стартовая машина — чистая
  `ZX-Evo BSconf`. SHA-256:
  `ZXMAK2.exe` —
  `903794B161E54215906C6D605092DDE0F7F7A3CBF442B2236CDEF7508626F5DF`;
  `ZXMAK2.Engine.dll` —
  `B28CF347F2BA871414596BF474E7CDEC87AD34B611E3E852F05990AF5CE0FFA7`;
  `ZXMAK2.Hardware.dll` —
  `D7C9E00667F8FBCBA2FCB32F2A2A5BB337192E6C50D1C73F67FA218AA61FD7E5`;
  `ZXMAK2.Host.dll` —
  `AB620EA438DA525700DC154DB7F3B350F6882E28CB6FFFE68654D06401339EC6`;
  `ZXMAK2.Host.Presentation.dll` —
  `9FFE8056D2C0CF6DE2ED8A671B16FDCA96382A8CF9A4077B9032DE7428671BE2`;
  `ZXMAK2.Host.WinForms.dll` —
  `F612B0CEF0ED95E851C86B470C471DB0FBB3848F2D4B6E318BA2BBFC50C67F3E`;
  `ZXMAK2.Mvvm.dll` —
  `832AD4893035AE3567861853219045E2E9CE624D8D3BF69E804524278CF74C60`;
  `Test.exe` —
  `7D2EDA589164E852542863BD0BBBAA95A5D029CE4B5950049382A02E8E9B1989`.

## Автоматический DirectSound-буфер — незавершённый рабочий этап — 2026-09-24

Статус: **код есть только в рабочей копии; этап не завершён, не
принят пользователем и не вошёл в portable-сборку.** Не считать его
частью v41 или кандидата Alpha 8 до отдельной приёмки.

- В `unity.config` значение `bufferCount=0` включает авторежим. Он
  начинается с четырёх 20-мс кадров (80 мс) и может увеличить
  буфер до пяти или шести кадров только после подтверждённых
  underrun. Пауза, остановленная VM и начальное заполнение не должны
  ложно увеличивать буфер. Во время игры буфер не уменьшается.
- Изменены `DirectSound.cs`, `unity.config` и
  `DirectSoundBufferProbe.cs`. Release-проект с этими правками собрался
  без ошибок; остались два старых warning о `.ruleset`.
- До завершения этапа нужны: финальный probe, чистая отдельная
  portable-папка, runtime-проверка без треска/пропусков и замер задержки.

## NeoGS: сверка с Unreal Speccy и план точечных улучшений — 2026-09-24

Статус: **только аудит и backlog; по итогам аудита код NeoGS не
менялся.** Текущая NeoGS Rev. C-VS принята runtime-проверкой:
NeoDOS определяет плату, MOD и MP3 играют, обе SD-карты и их
индикаторы работают, проведена пользовательская стресс-проверка.
Не переписывать рабочую плату по старому Unreal без конкретного
воспроизводимого дефекта.

### Источники сверки

- Локальный пакет `L:\Work_two\ZX\Unreal_SAA_Fix`, включая
  `Unreal.ini` и `doc\history.txt`.
- Unreal Speccy `pentevo/tools/unreal_fix/0.39.0/Unreal_NS` из WebSVN:
  `gsz80.cpp`, `gsz80.inl`, `vs1001.cpp/.h`, `sdcard.cpp/.h`.
- Старая реализация Unreal сама описана авторами как
  неполная. MP3 там декодируется внешней `bass.dll`, а NeoGS
  работает только в low-level Z80-режиме.

### Что уже есть у нас и не требует замены

- Последняя выбранная ревизия Rev. C-VS, официальная ROM 1.11,
  внутренний Z80, фиксированные 4 МБ RAM, 4/8-канальный DAC, DMA,
  writable microSD и VS1011/MP3.
- Исполнение прошивки и обмен с основной машиной идут в одном
  детерминированном времени, без старой фоновой Z80-гонки.
- VS1011 имеет ограниченные очереди, DREQ зависит от фактического
  потребления потока, а аппаратный/программный reset и корректная
  остановка декодера уже реализованы через managed NLayer.
- Текущие 4 МБ соответствуют нашей целевой последней ревизии;
  переключатель 2/4 МБ из Unreal не добавлять без отдельной
  потребности в совместимости со старыми платами.

### Что планируется сделать после отдельной команды

1. Добавить regression-тест сброса NeoGS во время MOD, MP3 и SD-обмена:
   проверять синхронизацию обоих Z80, стартовые банки памяти,
   порты, повторный запуск прошивки и отсутствие зависшего звука.
2. Расширить VS1011-тесты: hardware/software reset при заполненном
   буфере, три состояния DREQ, повторный MP3 после reset, а также
   регистры `STATUS`, `VOL`, `DECODE_TIME` и `HDAT`.
3. Расширить SD/DMA-тесты: `CMD9`, `CMD10`, запись сектора,
   многоблочное чтение и одновременную работу SD Z-controller и SD NeoGS.
4. Отдельно исследовать временную задержку маркера данных SD. В
   Unreal используется `SDDelay=1500` тактов 3,5 МГц; у нас сейчас
   есть только один промежуточный `FF`. Если точная задержка нужна,
   реализовать её в обвязке NeoGS, не меняя вслепую общий `SdCard`,
   которым уже пользуется и Z-controller. Включать это только после
   конкретного compatibility-теста, так как текущие карты уже приняты.

### Что не переносить из Unreal_SAA_Fix

- `SAASound.dll` не относится к NeoGS; это библиотека SAA1099 для
  TurboSound FM Pro.
- Не добавлять внешнюю `bass.dll` и не переносить высокоуровневый
  BASS-режим, обходящий внутренний Z80 и родную прошивку.
- `GSVol=8000` и `BASSVol=8000` — обычные коэффициенты микшера, а не
  специальная обработка звука. Не менять принятые уровни NeoGS без
  слухового и измерительного основания.
- Не заменять текущую SD-модель старой read-only реализацией Unreal.

## AY/YM headroom correction — v42 — 2026-09-24

Статус: **пользовательская runtime-приёмка пройдена; v42 назначена стабильной
контрольной сборкой и базой для отката.** Проверенная portable-папка v41 также
сохранена без изменений.

- Локализована регрессия обычной AY-музыки через TurboSound FM Pro. AYCHRV,
  профиль BaseConf и маршрутизация Music между v40 и v41 не менялись; дефект
  появился из-за добавленного в v41 общего post-gain `+50%` после рендера двух
  SSG YM2203. Их AY/YM-таблица уже использует полный signed-диапазон, поэтому
  повторное усиление отдельно насыщало SSG до общего микшера и разрушало
  динамические соотношения каналов/огибающих.
- Исправление узкое: SSG D1/D2 возвращены к принятому в v40 native-уровню.
  Усиление `+50%` SAA1099 и FM сохранено. Интерфейс и вся функциональность v41
  не менялись; незавершённый автоматический DirectSound-буфер в сборку не
  попал.
- Усилен `Test.exe /tsfm`: D1 и D2 получают настоящие tone/mixer/volume
  регистры, измеряются пики и насыщенные стереоотсчёты, добавлен обязательный
  `AY/YM SSG native headroom`. На исходном `ZXMAK2.Hardware.dll` v41 тест
  воспроизводимо падает: D1/D2 `1764/1764` saturated samples. На исправленном
  DLL проходит: D1 `811/1764`, D2 `844/1764`; D1, D2, FM и SAA дают звук.
- Полная Release-сборка: 0 ошибок, два прежних предупреждения `.ruleset`.
  Прошли полный `Test.exe`, `Test.exe /tsfm`, `PentEvoProfileProbe` и
  `NeoGsProbe`.
- Чистая portable-папка:
  `L:\Work_two\ZX\ZXMAK2-v42-ZXEVO-BC-Alpha8-AYFix-Test-20260924\release`.
  51 файл, 9507606 байт. По сравнению с v41 изменены только
  `ZXMAK2.Hardware.dll`, `Test.exe` и README; `ZXMAK2.exe`,
  `ZXMAK2.Host.WinForms.dll`, конфигурации, ROM и все остальные файлы совпадают.
  Пользовательских `.cmos`, `.nvram`, `.vmide`/`.vide`, `.vmz`, PDB, образов и
  логов в пакете нет. SHA-256: `ZXMAK2.Hardware.dll` —
  `8F764FDA6BDD2D7039CA5ABEEC0B1FB165E3229798FDE04AC52044111B4BFE90`;
  `Test.exe` —
  `BCBFF1FD2BE07CE0751C2738695D6AE6A0D6596B525AD13DC36ED16504898CB3`.

### Пользовательская приёмка v42 — 2026-09-24

- Пользователь проверил исправленный AY/YM-тракт и не обнаружил новых проблем:
  «вроде всё хорошо». По отдельному решению v42 сохраняется как стабильная
  сборка. Её portable-папку больше не изменять; все дальнейшие опыты начинать
  только с отдельной копии.
- Незавершённый автоматический DirectSound-буфер не является частью v42.
  Публикация Alpha 8 на GitHub выполняется только по отдельной команде.

## Актуальный список дальнейших работ после стабильной v42 — 2026-09-24

Порядок ниже отделяет ближайшие работы от самостоятельных будущих подсистем.
Закрытые Tape, FDD/HDD/SD lifecycle, ZXBUS foundation, NeoGS runtime, TSFM,
BaseConf CD/Time Gal и B40 video/timing повторно не открывать без нового
воспроизводимого дефекта.

### Ближайшие отдельные этапы

1. **Оформление Alpha 8 из стабильной v42.** Очистить portable-пакет по уже
   принятому правилу и опубликовать GitHub prerelease только по прямой команде.
2. **Автоматический DirectSound-буфер.** Закончить уже начатый изолированный
   эксперимент: финальный probe, отдельная сборка от v42, проверка треска,
   пропусков и задержки. Не переносить в стабильную ветку до runtime-приёмки.
3. **NeoGS hardening без переписывания принятой платы.** Добавить тесты reset
   во время MOD/MP3/SD, регистров VS1011, SD/DMA и одновременной работы двух
   SD-карт. Временную задержку SD менять только при конкретном тесте
   совместимости. Это техническое укрепление, а не открытая пользовательская
   ошибка NeoGS.
4. **Проверка `SAASound.dll` как эталона SAA1099.** Отдельно разобрать
   `L:\Work_two\ZX\Unreal_SAA_Fix\SAASound.dll`: установить точную версию,
   разрядность, происхождение, лицензию и интерфейс; затем сделать A/B-сравнение
   регистров, генераторов тона и шума, огибающих, таймингов и выходного уровня с
   нашим управляемым `TsFmSaa1099Renderer`. Текущий принятый SAA1099 вслепую не
   заменять и внешнюю DLL сразу в релиз не добавлять. Сначала — отдельный probe
   и тестовая сборка от v42; если библиотека непригодна как зависимость,
   переносить только проверенные алгоритмы и граничные случаи с учётом лицензии.
   Этот этап относится к TurboSound FM Pro и будущему MultiSound, но не к NeoGS.

### Самостоятельные будущие функции

5. **ZX-MultiSound — только самая новая доступная ревизия.** На дату записи
   официальный актуальный вариант — Rev.A2, но перед началом снова проверить
   репозиторий, CPLD, ROM и прошивки. Реализовать одной платой ZXBUS, а не
   набором несвязанных устройств: TurboSound FM (2×YM2203), General Sound
   16 МГц/1 МБ, SAA1099 с портами именно MultiSound, четырёхканальный SounDrive
   и MIDI/SAM2695. Уже готовы ZXBUS, TSFM, SAA, звуковой микшер и опыт NeoGS;
   отдельно требуются точный CPLD-декодер, GS-конфигурация MultiSound,
   SounDrive и наиболее сложная часть — достоверный SAM2695. Работать только в
   отдельной экспериментальной сборке от v42.
6. **ULAplus для BaseConf.** Сделать отдельным этапом по официальным портам
   `#3B`; не смешивать с принятой 4:4:4-палитрой и не переписывать B40
   video/timing без конкретного расхождения.
7. **Утилита создания носителей.** IMA/IMG/raw/VHD/HDD, выбор размера и layout,
   подготовка FAT32 или MBR+FAT32, запись файлов и проверка чтением/хэшами.
   Эталонные параметры уже закреплены выше в этом журнале.
8. **ATM Turbo 2+ — отдельный цикл.** Не переносить туда BaseConf-решения
   вслепую; отдельно ревизовать исходную эмуляцию машины и её CD/NedoVIDEO.
9. **Автозагрузка NedoOS с HDD.** Отдельная функция удобства, а не дефект
   принятого IDE-контроллера.

## ZX-MultiSound Rev.A2 — v43 experimental — 2026-09-24

Статус: **реализация и автоматическая проверка завершены; требуется
пользовательская runtime-приёмка. Это отдельный локальный кандидат, не GitHub
Alpha 8. Стабильная v42 и её portable-папка не изменялись.** Работа выполнена
в изолированном worktree от принятого коммита v42 `aaf78b7`.

### Зафиксированная аппаратная база

- Последняя публичная ревизия платы на дату проверки — ZX-MultiSound Rev.A2,
  официальный репозиторий `UzixLS/zx-multisound`, commit
  `d7f3ac293724e1b69cbe9af9894311de4e07e4d1` от 2026-06-17.
- Для внутреннего General Sound взят официальный ROM `gs-firmware` v1.05b:
  32768 байт, SHA-256
  `9948EC9617365FB1913B0C56E00D5F6B6A36084875CCA16DEBAE270A4C6E6ADF`.
- Плата реализована одним ZXBUS-устройством: 2×YM2203/TSFM, SAA1099,
  GS 16 МГц/1 МиБ, четыре DAC SounDrive. Декодирование портов, reset,
  ROM/RAM-карта GS, флаги `#B3/#BB`, громкости DAC, точный 321-тактовый
  счётчик INT и блокировка SAA/SounDrive после M1 в `#0000-#3FFF` перенесены
  из официального `cpld/rtl/top.v`.
- Важная честная граница: внешний SAM2695 — закрытый wavetable-синтезатор.
  Публичные исходники платы дают его тактирование и электрическое подключение,
  но не содержат исполняемого ядра, прошивки или wave ROM. В v43 MIDI/SAM2695
  звук не эмулируется и завершённым не считается; остальные перечисленные
  цифровые блоки работают независимо от него.

### ZXBUS, интерфейс и защита конфликтов

- Оба физических слота равноправны. NeoGS и MultiSound разрешены одновременно
  в любом порядке; две одинаковые платы запрещены.
- В Machine Settings добавлена отдельная страница `ZXBUS PentEvo`; настройки
  больше не находятся под ULA. На ней два слота и четыре реальных переключателя
  MultiSound: YM/TSFM, SAA, GS, SounDrive.
- Automatic включён по умолчанию. При NeoGS блок GS MultiSound выключается,
  потому что обе платы декодируют `#B3/#BB`; SAA/SounDrive/YM остаются. При
  активном YM MultiSound внутренний Music переводится в `None`, чтобы убрать
  конфликт AY-портов. Ручной режим валидируется до изменения bus/profile и
  сообщает точный конфликт, не оставляя полуизменённую конфигурацию.
- Страница `Internal PentEvo` теперь имеет `None`, `AY/YM (AY8910-CHRV)` и
  `TurboSound FM Pro`; состояние и громкость сохраняются в XML. Принятые v42
  UI, панели носителей, NeoGS и TSFM не переписывались.

### Проверка

- Полная Release-сборка `ZXVM.sln`: 0 ошибок, только два прежних предупреждения
  об отсутствующих `.ruleset`.
- `Test.exe /multisound`: partial YM aliases, SAA, GS ROM 1.05b handshake,
  SounDrive, ROM-M1 lock, automatic conflict policy и manual guard — PASS.
- `PentEvoProfileProbe`: NeoGS+MultiSound в обоих порядках слотов, запрет
  дублей, отклонение ручного конфликта до мутации и XML round-trip — PASS.
- `NeoGsProbe` и `Test.exe /tsfm` — PASS. Полный `Test.exe`: все ULA sanity
  PASS; `testVideo.z80` 360-366/339-352 мс, `testOutFe.z80` 388-417 мс,
  light/cpu 256-265 мс. Регрессии рендеринга на несколько секунд нет.
- Подробная трассировка решений и источников находится в
  `docs/ZX-MULTISOUND-A2-IMPLEMENTATION.md`; release note —
  `docs/releases/README-B43.txt`.
- Отдельная чистая portable-папка для runtime-приёмки:
  `L:\Work_two\ZX\ZXMAK2-v43-ZXEVO-BC-Alpha8-MultiSound-A2-Test-20260924\release`.
  51 файл, 9558932 байта. По сравнению со стабильной v42 изменены только
  `ZXMAK2.Hardware.dll`, `ZXMAK2.Host.WinForms.dll`, `Test.exe` и README.
  Пользовательских `.cmos`, `.nvram`, `.vmide`/`.vide`, `.vmz`, PDB, образов
  и логов нет. Контрольный `Test.exe /multisound` запущен непосредственно из
  portable-папки и прошёл. SHA-256: Hardware —
  `0442845F0458FB0F75355EA7CF12D7A5AEDE7D666505C842EC7E4EB7F8047C31`;
  Host.WinForms —
  `372661434C3116025FCC4B5842FDE2B6F6C2DBC4430D30BBB8A4AEED9E1DC606`;
  Test —
  `E188DCDAF05C13EAF54DD75D22A57C56A6A71C867D8F6C10A262C31CE51EB60C`.

## ZX-MultiSound Rev.A2 audio correction — v44 test — 2026-09-24

Статус: **исправление реализовано и прошло автоматические тесты; требуется
пользовательская runtime-приёмка на тех же SAA, GS и SounDrive программах,
которые выявили дефекты v43.** Стабильная v42 и контрольная portable-папка v43
не изменялись. GitHub-релиз не создавался.

### Что воспроизвёл пользователь

- SAA1099 на MultiSound выдавал узнаваемую, но неполную и «хрюкающую» музыку.
- Проверенный General Sound софт не давал звука.
- Проверенный SounDrive софт также не давал звука.

### Найденные причины и исправления

- General Sound исполняет отдельный Z80 на 16 МГц пакетами относительно
  основного Z80. Все изменения DAC внутри пакета получали одну и ту же метку
  времени основного процессора; `SoundDeviceBase` по контракту отбрасывал
  повторные нерастущие метки. Теперь каждое обращение к DAC получает время из
  собственного хода GS Z80. Перед записью SounDrive GS синхронизируется до
  текущей позиции основной машины, поскольку оба источника физически делят
  четыре DAC-регистра.
- Длина активного низкого уровня GS INT уточнена непосредственно по
  `cpld/rtl/top.v`: 33, а не 32 такта 12 МГц (фронт reload плюс старые значения
  счётчика 0..31); полный период остаётся 321 такт.
- Старое управляемое ядро SAA1099 сверено с актуальным исходным SAASound 3.5,
  commit `3d92322030d1e5627cbf8295bed1efc872bad6eb`. Перенесены проверенные
  правила физического смешивания tone/noise, буферизации offset/octave на
  границе полупериода, режима noise=3, 18-битного Galois LFSR, огибающих,
  3/4-битного разрешения, PDM-амплитуды и sync/reset. Добавлен штатный для
  современной библиотеки 64x oversampling.
- Внешняя пользовательская `SAASound.dll` не включена: она x86 и создала бы
  отдельную платформенную зависимость. Использован её актуальный открытый
  первоисточник; существующий managed-тракт сохранён.

### Автоматическая проверка

- Полная Release-сборка: 0 ошибок, два прежних warning `.ruleset`.
- `Test.exe /multisound`: базовые порты/ROM/policy PASS; отдельный GS waveform
  дал 301 смену знака, SounDrive waveform — 101. Эти тесты падают, если поток
  снова схлопывается в одиночный щелчок.
- `Test.exe /tsfm`: D1/D2/FM/SAA PASS; отдельно проверены все шесть тональных
  каналов SAA, независимый шум и огибающая. Пики: шесть раз 8136, noise 8136,
  envelope 7186; клиппинга SAA нет.
- `NeoGsProbe`: PASS. Полный `Test.exe`: все ULA sanity PASS; 500 кадров
  `testVideo` 373-385 мс, `zexall` 338-352 мс, `testOutFe` 396-410 мс,
  light 263-269 мс, CPU 215-230 мс.

Подробное техническое описание продолжено в
`docs/ZX-MULTISOUND-A2-IMPLEMENTATION.md`; памятка тестовой сборки —
`docs/releases/README-B44.txt`.

### Отдельная portable-сборка v44

- Папка для пользовательской проверки:
  `L:\Work_two\ZX\ZXMAK2-v44-ZXEVO-BC-Alpha8-MultiSound-AudioFix-Test-20260924\release`.
- 51 файл, 9563537 байт. Пакет создан копированием принятой структуры v43;
  заменены только `ZXMAK2.Hardware.dll`, `Test.exe` и README. Поэтому весь
  принятый интерфейс v43 сохранён байт-в-байт. Запрещённых пользовательских
  `.cmos`, `.nvram`, `.vmide`/`.vide`, `.vmz`, PDB, образов и логов нет.
- `Test.exe /tsfm`, `Test.exe /multisound` и полный `Test.exe` повторно
  запущены непосредственно из portable-папки и прошли.
- SHA-256: `ZXMAK2.Hardware.dll` —
  `23C891FCF9AA3C8C8429CBF5BC9BC047790A1749F8CB8391DBFD06D8A77DBDED`;
  `Test.exe` —
  `C1E849B367533F0A9F392F8DB71691E9E5B5762126557F51B1D992A37A006A9C`;
  неизменённый `ZXMAK2.Host.WinForms.dll` —
  `372661434C3116025FCC4B5842FDE2B6F6C2DBC4430D30BBB8A4AEED9E1DC606`.

## ZX-MultiSound SAA register-bus correction — v45 test — 2026-09-24

Статус: **точная причина оставшихся искажений SAA найдена в адаптере платы и
исправлена в отдельной ветке `codex/multisound-saa1099-fix`; требуется
пользовательская проверка той же доверенной композиции.** Принятая v44
зафиксирована отдельным локальным коммитом `a438813` и не изменялась.

### Причина

- Реальная схема из официального `cpld/rtl/top.v` формирует `saa_cs_n` по
  записи в `#FF/#1FF`, аппаратному переключателю SAA и ROM-M1 lock. Состояние
  `saa_clk_en` в выборе регистров вообще не участвует.
- Поэтому корректный проигрыватель имеет право при остановленном 8-МГц такте
  сначала записать все регистры SAA, а затем запустить генераторы командой
  `#F7` через любой подходящий псевдопорт `#FFFD`.
- В v43/v44 адаптер ошибочно считал выключенный такт отключением всей
  микросхемы и выбрасывал такие записи. После запуска сохранялась лишь та
  часть состояния, которую проигрыватель менял повторно: отсюда узнаваемая,
  но неполная музыка с хрипами и помехами. Синтетический тест v44 включал такт
  до записи регистров и потому эту ошибку не обнаруживал.

### Исправление и защита

- Доступ к регистрам и внешний такт разделены: `#FF/#1FF` принимаются при
  остановленном такте, а генераторы при этом не продвигаются. `#F7/#FF`
  управляют именно ходом внешнего 8-МГц такта.
- Новый board-level тест записывает полную конфигурацию одного голоса до
  `#F7`, затем запускает такт и требует звуковой поток. На старом коде эта
  последовательность даёт тишину и тест падает.
- `Test.exe /multisound` и `Test.exe /tsfm` после правки проходят; принятые
  TSFM Pro, GS, SounDrive, AY/YM, NeoGS и интерфейс не переписывались.
- Сверка выполнена с ZX-MultiSound Rev.A2 commit `d7f3ac2`, актуальным
  SAASound 3.5 commit `3d92322` и реализацией Unreal Speccy commit `9dae221`.
  Внешняя `SAASound.dll` по-прежнему не добавляется.

### Отдельная portable-сборка v45

- Папка:
  `L:\Work_two\ZX\ZXMAK2-v45-ZXEVO-BC-Alpha8-MultiSound-SAA-BusFix-Test-20260924\release`.
  Она создана из принятой структуры v44; заменены только
  `ZXMAK2.Hardware.dll`, `Test.exe` и README. Интерфейсный
  `ZXMAK2.Host.WinForms.dll` остался байт-в-байт прежним.
- 51 файл, 9563670 байт. Пользовательских `.cmos`, `.nvram`,
  `.vmide`/`.vide`, `.vmz`, PDB, образов и логов нет.
- Непосредственно из portable-папки прошли `Test.exe /multisound`,
  `Test.exe /tsfm` и полный `Test.exe`; полная Release-пересборка до упаковки
  также завершилась без ошибок с двумя прежними warning `.ruleset`.
- SHA-256: `ZXMAK2.Hardware.dll` —
  `E3957A7FC63091C8F6B6EE185795795CB4D1F97B8A95864F9171AD4295FB5AC2`;
  `Test.exe` —
  `DAB3E2245F6A273C0998C82010C9906921201A08B6D46581C9A2C5A58DE7B1D0`;
  неизменённый `ZXMAK2.Host.WinForms.dll` —
  `372661434C3116025FCC4B5842FDE2B6F6C2DBC4430D30BBB8A4AEED9E1DC606`.

## ZX-MultiSound runtime audio rollback — v46 test — 2026-09-24

Статус: **после пользовательского сравнения с реальной записью найдено
неудачное направление v44/v45: новый экспериментальный SAA-рендерер проходил
синтетические векторы, но искажал настоящую музыку. В отдельной ветке
`codex/multisound-runtime-audio-fix` восстановено принятое в v42 звуковое ядро
SAA; исправление шины и остановки такта Rev.A2 сохранено.**

### Что изменено

- `TsFmSaa1099Renderer` возвращён ровно к принятой реализации v42. Сверка
  `git diff 1561282` показывает только одно намеренное расширение: свойство
  `ClockEnabled` и остановку генераторов при выключенном внешнем такте.
- Таким образом убрана крупная перепись v44 (oversampling, новая модель
  tone/noise/envelope), которая выглядела убедительно в искусственных тестах,
  но дала кашу и неполную мелодию на реальном SAA-софте.
- Корректный фикс v45 не потерян: регистры SAA по-прежнему принимают записи
  при остановленном такте, как требует официальный `top.v`; замораживается
  только продвижение генераторов.
- `/multisound` усилен: теперь он независимо программирует и требует звук от
  обоих физических SSG — D1 и D2 — через реальные частично декодированные
  адреса. Старый тест проверял лишь один регистр и мог пропустить полностью
  немой второй чип.

### Проверка

- Полная Release-сборка: 0 ошибок, два прежних warning `.ruleset`.
- `Test.exe /multisound`: D1=True, D2=True, SAA-PRELOAD=True, GS и SounDrive
  waveform PASS, ROM lock и конфликтная политика PASS.
- `Test.exe /tsfm`: оба YM2203 SSG, FM, шесть SAA-каналов, noise и envelope
  PASS. Полный `Test.exe`: все ULA sanity PASS; 500 кадров `testVideo`
  367-383 мс, `zexall` 339-357 мс, `testOutFe` 399-406 мс, light 259-264 мс,
  CPU 232-252 мс.
- Следующий обязательный этап — пользовательская проверка той же доверенной
  SAA-композиции и насыщенности TurboSound на настоящем программном материале.
  Синтетический PASS больше не считается достаточной приёмкой качества звука.

### Отдельная portable-сборка v46

- Папка:
  `L:\Work_two\ZX\ZXMAK2-v46-ZXEVO-BC-Alpha8-MultiSound-RuntimeAudioFix-Test-20260924\release`.
  Она создана из структуры v45; изменены ровно `ZXMAK2.Hardware.dll`,
  `Test.exe` и README. Интерфейсный `ZXMAK2.Host.WinForms.dll` остался
  байт-в-байт прежним.
- 51 файл, 9561618 байт. Пользовательских `.cmos`, `.nvram`,
  `.vmide`/`.vide`, `.vmz`, PDB, образов и логов нет.
- Непосредственно из portable-папки прошли `Test.exe /multisound`,
  `Test.exe /tsfm` и полный `Test.exe`.
- SHA-256: `ZXMAK2.Hardware.dll` —
  `7CAEA5FA179367684805E50B801EB4365F36BA47219D7B0DB18CC21CE2A06500`;
  `Test.exe` —
  `BAF275F0912EB3FB1AA62EF31E61BAABC416FAD2CD21EA518D015B6280E3F159`;
  неизменённый `ZXMAK2.Host.WinForms.dll` —
  `372661434C3116025FCC4B5842FDE2B6F6C2DBC4430D30BBB8A4AEED9E1DC606`.
...
Статус: **реализованы два явных режима совместимости портов, Release-сборка,
автоматические тесты и пользовательская проверка обоих предоставленных TRD
прошли. v47 зафиксирована локально как стабильный кандидат Alpha 8; публикация
на GitHub отложена по решению пользователя.** Принятая v46 и её
portable-папка не изменяются.
...
## ZXM-MoonSound Rev.01 — v49 test — 2026-09-25

Статус: **отдельная локальная проверочная сборка на базе неизменённой v48.
ZXM-MoonSound Rev.01 реализована согласно опубликованной схеме платы и CPLD
1.00; на GitHub сборка не публиковалась.** Ветка разработки:
`codex/moonsound-rev01`.

### Аппаратная модель

- В ZXBUS-список ZX Evolution BaseConf добавлена `ZXM-MoonSound Rev.01`.
  Плату можно поставить в Slot 1 или Slot 2; оба разъёма равноправны.
- Повторная установка второй MoonSound запрещена, потому что обе платы
  декодировали бы одинаковые фиксированные порты. С NeoGS или ZX-MultiSound
  она сосуществует в пределах двух физических слотов.
- Дополнительных галочек и вымышленных джамперов нет: у Rev.01 они не нужны.
- По официальному `dd2.tdf` CPLD 1.00 реализовано декодирование младшего байта:
  `#7E/#7F` для PCM и `#C4-#C7` для FM. Старший байт адреса игнорируется.
- Учтён сигнал `ENDOS`: при включённых портах PentEvo TR-DOS плата освобождает
  обе группы портов, как физическая Rev.01.
- Память соответствует плате: 2 МБ YRW801-M ROM и ровно 1 МБ SRAM. Старое
  расширение некоторых эмуляторов до 4 МБ не копировалось.

### Звуковое ядро и ROM

- Для полного YMF278B/OPL4 взято актуальное BSD-3-Clause ядро `ymfm` Aaron
  Giles: OPL3 FM и 24-канальная wavetable/PCM часть работают одним чипом на
  33,8688 МГц с родным выходом 44,1 кГц.
- Исходники, лицензия и небольшой native C ABI bridge находятся в
  `src/Libraries/YmfmOpl4`. В пакет добавлены `ymfm_opl4.dll` и
  `ymfm-opl4.LICENSE.txt`.
- Регистровые записи привязаны к текущему месту кадра; перед каждой записью
  рендерится уже прошедшая часть, поэтому mid-frame sound updates не
  сдвигаются на границу кадра.
- Yamaha YRW801-M ROM не занесён в Git. Локальная тестовая папка содержит уже
  имевшуюся у пользователя 2-МБ копию с SHA-256
  `0481E861F639FA3D6A64A75B735035A54B0875D57AAAE33FDBE400146C80423F`.
  Перед любой публичной публикацией этот ROM надо удалить, если не будет
  подтверждено право распространения.

### Проверка

- Полная Release-пересборка: 0 ошибок, два прежних warning отсутствующих
  `.ruleset`.
- `Test.exe /moonsound`: OPL4 status `#06/#02`, SRAM `#5A`, FM peak 8145,
  PCM peak 8124, TR-DOS gate PASS.
- `Test.exe /tsfm` и `Test.exe /multisound`: все прежние звуковые тракты,
  переключатели совместимости и конфликтная защита PASS.
- BaseConf UI probe: MoonSound в любом слоте, совместная установка, запрет
  дубля, отсутствие обрезки и XML round-trip — PASS.
- Полный `Test.exe`: все ULA sanity PASS; 500 кадров из portable-папки:
  `testVideo` 379–385 мс, `zexall` 342–350 мс, `testOutFe` 404–411 мс,
  light 271–273 мс, CPU 240–242 мс. Регрессии замедления нет.

### Отдельная portable-сборка v49

- Папка:
  `L:\Work_two\ZX\ZXMAK2-v49-ZXEVO-BC-Alpha8-MoonSound-Rev01-Test-20260925\release`.
  v48 не изменена.
- 56 файлов, 12387162 байта; пользовательских `.cmos`, `.nvram`,
  `.vmide`/`.vide`, `.vmz`, PDB и логов нет.
- `ZXMAK2.exe` и `Test.exe` намеренно 32-битные: сборка сохраняет 32-битную
  SAASound.dll из v48 и использует 32-битный bridge ymfm.
- SHA-256: `ZXMAK2.Hardware.dll` —
  `571C79CD12B9FAF04BAF09AC790D0D9B872F8E9AC21D6B6AC55D559307E955B0`;
  `ZXMAK2.Host.WinForms.dll` —
  `80A19DDB169CBFD0FFB29D5E396B039D473C1A6D8B33C6BA564BCBCDBBE2C826`;
  `Test.exe` —
  `238B76E37456879E7A49B0222910BAB707D99669BAF30EE70EF2A06622061E01`;
  `ymfm_opl4.dll` —
  `67E227E2B5500A750679D3FFA9C7EC5CF5D6458DDFBA81525EA855E6AB401C6F`.

Подробная техническая запись:
`docs/ZXM-MOONSOUND-REV01-IMPLEMENTATION.md`; памятка сборки:
`docs/releases/README-B49.txt`.

## ZXNetUSB Rev.C Ethernet and reset menu — v50 test — 2026-09-25

Статус: **принято пользователем как ZX Evolution BaseConf Beta 1. Исходный
код и portable-пакет опубликованы на GitHub под тегом `v0.8-beta1`.** Ветка
разработки: `codex/zxnetusb-reset`.

- В оба равноправных слота ZXBUS PentEvo добавлена плата
  `ZXNetUSB Rev.C (Ethernet)`. Вторую такую же плату одновременно выбрать
  нельзя, как и остальные платы с фиксированным декодированием портов.
- Реализована именно сетевая половина серийной Rev.C: CPLD-порты
  `#81AB/#82AB/#83AB`, полный I/O-диапазон W5300 `#00AB..#7FAB`, инверсия
  `ADDR[0]`, reset и идентификатор W5300 `#5300`. USB-контроллер SL811HS
  намеренно не подключён к USB компьютера и не эмулируется.
- Аппаратные сокеты W5300 отображаются на обычные непривилегированные сокеты
  Windows. Реализованы TCP client/server, UDP, формат FIFO W5300, состояния,
  команды и IRQ-флаги, необходимые текущему драйверу NedoOS.
- Для простого запуска без TAP/драйверов/прав администратора встроен маленький
  DHCP/NAT-контур: гостю выдаётся `10.0.2.15`, шлюз `10.0.2.2`, DNS
  `10.0.2.3`; DNS-запросы передаются DNS-серверу активного подключения
  Windows. IPRAW echo поддержан для диагностической проверки.
- Memory-mapped режим W5300 и USB-половина платы в эту узкую интернет-задачу
  не входят: NedoOS использует реализованный I/O-mapped драйвер W5300.
- Меню VM теперь явно показывает три уже существовавших действия:
  `Warm Reset — F12`, `CMOS Reset — Ctrl+F12`,
  `Factory Reset — Ctrl+Alt+F12`. Подсказка кнопки Warm Reset сокращена до
  `F12`; обработчики клавиш и сама логика сбросов не переписывались.
- Автотест `Test.exe /zxnetusb` проверяет доступ к ID W5300 настоящей
  программой Z80 через диспетчер I/O-портов эмулятора, реальную двустороннюю
  TCP-передачу через loopback Windows и ответ DHCP — PASS. Отдельно повторно
  прошли `/tsfm`, `/multisound` и `/moonsound`.
- Полный `Test.exe`: все ULA sanity PASS; 500 кадров из portable-папки:
  `testVideo` 377–390 мс, `zexall` 341–349 мс, `testOutFe` 402–405 мс,
  light 271–288 мс, CPU 243–248 мс. Возврата старого замедления нет.
- Отдельная папка:
  `L:\Work_two\ZX\ZXMAK2-v50-ZXEVO-BC-Alpha8-ZXNetUSB-RevC-Test-20260925\release`.
  В ней 56 файлов, 12402359 байт; пользовательских `.cmos`, `.vmide`/`.vide`,
  `.vmz` и PDB нет. SHA-256: `ZXMAK2.Hardware.dll` —
  `191430EEB359475D619513DB55E6F9CF8964058ED8617D806FDDAA567141F730`;
  `ZXMAK2.Host.WinForms.dll` —
  `D078C1719702D6EFF8D67987B805339DE79A1511A1FD9CD2A3DF76E42584A705`;
  `Test.exe` —
  `D010A20321E48840132D0CD85B7EAA45547ACE953A3EFB70A550620A62A46A45`.
- Пользовательская runtime-приёмка: браузер NedoOS через ZXNetUSB получил
  страницу `zxdn.narod.ru` и список ссылок. Искажённый русский текст —
  ограничение кодировки браузера; сетевой транспорт, DNS и HTTP-получение
