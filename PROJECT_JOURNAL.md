...
- Предыдущая попытка с `Ctrl+B` отменена отдельным коммитом: эта комбинация
  ...
  загрузчика. `Alt+F9` не пришла в главное окно при фактической проверке,
  поэтому заменена на `Ctrl+Alt+F11`. Она перехватывается в `ProcessCmdKey`
  ещё до `KeyDown`, меню и виртуальной клавиатуры: это важно, так как F11
  входит в расширенную карту клавиш ZX Evo. Рядом с
  `No Border` в меню явно показано `Ctrl+Alt+F11`.
