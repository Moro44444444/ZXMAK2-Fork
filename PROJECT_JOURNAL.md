...
## No Border: Ctrl+B и Eject для ленты — 2026-09-22

- `View → Scale Mode → No Border` теперь явно показывает справа горячую
  клавишу `Ctrl+B`; та же комбинация переключает режим на лету в главном
  окне эмулятора.
- В окно `Tape` справа от кнопок перемотки, Play и параметров добавлена
  текстовая кнопка `Eject`. Она доступна только при загруженной ленте,
  останавливает воспроизведение и очищает загруженные блоки — без reset
  машины и без изменения настроек лентопротяжного устройства.
- Полная Release-сборка прошла с 0 ошибок; остаются только два прежних
  предупреждения об отсутствующих `.ruleset`. Штатный `Test.exe` успешно
  выполнил все ULA sanity-векторы и benchmark-набор. GitHub-релиз и
  portable-архив для этой малой локальной правки не создавались.
