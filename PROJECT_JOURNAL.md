...
- `TapeDevice` теперь сообщает множитель этой шкалы через документированную
  модель `ICpuClock`: BaseConf — 8, обычные машины — 1. Этот множитель
  применяется только к TAP/TZX и к порогу ROM-loader autodetect. WAV/CSW уже
  строятся в частоте машины и не менялись.
