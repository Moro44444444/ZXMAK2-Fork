...
- Крайним справа добавлен CD/DVD. Основной щелчок подключает доступный
  ...
  Eject и Settings. Eject полностью отключает и очищает привод. Обычный reset
  сохраняет подключение, factory reset его сбрасывает. Индикация использует те
  же три состояния: серый — устройство/привод недоступен, красный — подключён,
  но диск не готов, зелёный — диск готов к чтению.
...
- Полная Release-пересборка: 0 ошибок, только два прежних предупреждения о
  ...
  round-trip настроек и расположение/состояния новых toolbar-кнопок.
...
- Чистая portable-папка:
  ...
  51 файл, 9505009 байт; отсутствуют PDB, probes, логи, образы и пользовательские
  ...
  `ZXMAK2.Host.Presentation.dll` —
  `DBEE9E0D82C4178817957B01DF8B2F4746DD78AC9549C29C59066CF7D914F733`;
  `ZXMAK2.Host.WinForms.dll` —
  `58150CA7390A950B5671FB58256B8B87C3E1A31DDE95DA9B262DC9B5D7E47051`;
  `ZXMAK2.Mvvm.dll` —
