...
Статус: **отдельная локальная проверочная ветка и сборка на базе принятой
v49; v49 не изменяется. На GitHub не публиковалось.** Ветка разработки:
`codex/zxnetusb-reset`.
