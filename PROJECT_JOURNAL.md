...
4. **ZX-MultiSound — только самая новая доступная ревизия.** На дату записи
...
5. **ULAplus для BaseConf.** Сделать отдельным этапом по официальным портам
   `#3B`; не смешивать с принятой 4:4:4-палитрой и не переписывать B40
   video/timing без конкретного расхождения.
6. **Утилита создания носителей.** IMA/IMG/raw/VHD/HDD, выбор размера и layout,
...
7. **ATM Turbo 2+ — отдельный цикл.** Не переносить туда BaseConf-решения
   вслепую; отдельно ревизовать исходную эмуляцию машины и её CD/NedoVIDEO.
8. **Автозагрузка NedoOS с HDD.** Отдельная функция удобства, а не дефект
