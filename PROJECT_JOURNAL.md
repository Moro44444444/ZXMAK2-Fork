...
- Множитель задаётся явно в профиле, а не по общему интерфейсу часов:
  BaseConf — 8, все прочие машины по умолчанию — 1. Это устраняет регрессию,
  при которой ×8 ошибочно применялся к другим машинам с собственными часами.
...
- Дополнительно подтверждён реальный сценарий: при выборе `PENTAGON 128K`
  после BaseConf в сохранённом `ZXMAK2.vmz` сохранялся старый атрибут `8`.
  После BusInit `TapeDevice` проверяет наличие именно `UlaPentEvo`; при
  любой другой ULA такой устаревший атрибут принудительно читается как `1`.
