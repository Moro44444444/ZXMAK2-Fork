...
{
    public class MemoryAtm710 : MemoryBase, ICpuClock
    {
        #region Fields
        ...
        protected virtual void BusReadPortFE(ushort addr, ref byte value, ref bool handled)
        {
            // bit Z emulation
            value &= 0xDF;
            // The Z signal is generated by the video beam, so its period must
            // not change when the CPU switches between 3.5 and 7 MHz. Convert
            // fastest-clock master tacts back to the renderer time base.
            int frameLength = m_ula.FrameTactCount /
                MaxCpuClockMultiplier;
            int frameTact = (int)(m_cpu.Tact % m_ula.FrameTactCount) /
                MaxCpuClockMultiplier;
        ...
        public override void ResetState()
        {
            ...
            // Init 128K
            m_aFF77 = 0xFF77;
            // Power-on/snapshot initialization must match a hardware reset:
            // D3=0 means the normal 3.5 MHz clock.  The former #AB value had
            // D3 set and made ATM Turbo 2+ start at 7 MHz after a snapshot.
            m_pFF77 = 0x00A3;
            Array.Copy(
